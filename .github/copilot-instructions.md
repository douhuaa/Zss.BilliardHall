...
**强制要求**：
- ✅ **所有任务回复必须使用中文**（与用户的对话交流）
- ✅ **所有进度报告必须使用中文**（报告工作进度时）
- ✅ **所有 FailureObject 输出必须使用中文**
- ✅ **任务开始、执行中的响应必须使用中文**（与用户的交互）

**允许使用英文的场景**：
- ✅ **PR 标题和描述可以使用英文**（面向代码仓库的正式文档）
- ✅ **代码注释和文档可以使用英文**（遵循项目既有规范）
- ✅ **技术术语和 API 文档使用英文**

**禁止行为**：
- ❌ 禁止在与用户交互时使用英文（除非用户明确要求）
- ❌ 禁止在反馈报告中使用英文

> ⚠️ **重要说明**：本规则区分了内部沟通（使用中文）和对外文档（可使用英文）。PR 标题和描述作为代码仓库的正式记录，允许使用英文以保持项目的国际化兼容性。
...
> ⚠️ **语言要求**：内部反馈和进度更新必须使用中文。但 PR 标题和描述作为代码仓库的正式记录，可以使用英文。
...
> 🔴 **关键规则**：所有 Agent 在与用户交互时必须使用中文，但在生成 PR 标题和描述等代码仓库正式文档时可以使用英文。
> 
> **必须使用中文的场景**：
> - 任务开始时的响应
> - 执行过程中的进度更新（report_progress 的 commitMessage）
> - 与用户的所有对话和交互
...
> - 技术术语
> 
> **违规行为**：在与用户对话时使用英文是不可接受的行为（除非用户明确要求）。
