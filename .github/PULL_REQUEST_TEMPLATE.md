...
如果 Copilot Agent 生成的内容使用了英文：
1. 请手动修改为中文
2. 或要求 Copilot 重新生成（明确要求使用中文）

不符合语言要求的 PR 将被要求修改后才能合并。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
-->
...
**所有涉及 ADR 或架构测试变更的 PR 必须完成此部分**

### 自动化验证工具

- [ ] 本 PR 未修改 ADR 文档或架构测试（跳过此检查）
- [ ] 已运行 `./scripts/validate-adr-consistency.sh` 且通过（检查编号/目录/元数据一致性）
- [ ] 已运行 `./scripts/validate-three-way-mapping.sh` 且通过（检查 ADR/测试/Prompt 映射）
- [ ] 如有问题，已根据工具生成的修正清单进行修复

### ADR 变更类型（如适用）

- [ ] 新增 ADR（使用 `./scripts/adr-cli.sh create` 创建）
  - [ ] 已创建对应的测试文件（如标记【必须测试】）
  - [ ] 已创建对应的 Prompt 文件
- [ ] 修订 ADR 约束（已同步更新测试和 Prompt）
- [ ] 新增架构测试（已在 ADR 文档中标记对应条款）
- [ ] 修订架构测试（已验证与 ADR 约束一致）

### 三位一体映射验证

- [ ] 所有 ADR 必须测试的条款都有对应测试方法
- [ ] 所有测试断言都引用了具体 ADR 及条款编号
- [ ] 测试失败消息包含 ADR 引用和修复建议
- [ ] 每个 ADR 都有对应的 Copilot Prompt 文件
- [ ] Copilot 审查已确认测试与 ADR 内容无偏差

> 💡 **工具使用指南**：[scripts/README.md](../scripts/README.md)  
> 💡 **详细规范**：[ADR-测试映射规范](../docs/ADR-TEST-MAPPING-SPECIFICATION.md)
...
4. *
   *所有破例将被记录在 [`docs/summaries/governance/arch-violations.md`](../docs/summaries/governance/arch-violations.md)
   中以备审计**
