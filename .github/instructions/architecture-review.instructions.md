...
### ⚠️ 强制性步骤：运行架构测试（MANDATORY）

**在提供任何评审意见之前，必须运行架构测试。这是强制性的，不可跳过。**

#### 为什么这是强制性的

- ❌ **错误案例**：依赖 `code_review` 工具返回 "No changed files found" 就认为没有问题
- ❌ **错误案例**：认为纯文档 PR 不需要运行测试
- ❌ **错误案例**：假设没有代码变更就没有架构影响
...
```bash
# 步骤 1：必须运行架构测试
dotnet test src/tests/ArchitectureTests/ --filter "FullyQualifiedName~ADR"
