...
示例：
> "根据 ADR-0001（章节：模块通信），模块不得直接引用彼此。这由 `ADR_0001_Architecture_Tests.cs` 中的
`Modules_Should_Not_Reference_Other_Modules` 测试强制执行。正确模式请参见 `docs/copilot/adr-0001.prompts.md`（场景
> 3：模块通信）。"
