...
if [ ! -f "$VIOLATIONS_FILE" ]; then
    check_result "arch-violations.md 存在性" "FAIL" "文件不存在"
else
    # 检查必需章节
    required_sections=(
        "## 核心变更"
    ...
    if [ ${#missing_sections[@]} -eq 0 ]; then
        check_result "arch-violations.md 结构完整性" "PASS" "所有必需章节存在"
    else
        check_result "arch-violations.md 结构完整性" "FAIL" "缺失章节: ${missing_sections[*]}"
    fi
    
    # 检查强制字段（表头）
    if grep -q "| ID | ADR | 规则 | 违规位置 | 原因 | 到期版本 | 负责人 | 偿还计划 | 审批人 | 状态 |" "$VIOLATIONS_FILE"; then
        check_result "arch-violations.md 强制字段" "PASS" "包含到期版本、负责人、偿还计划等强制字段"
    else
        check_result "arch-violations.md 强制字段" "FAIL" "表头不符合 ADR-0000.Y 要求"
    fi
fi
...
if [ ${#missing_workflows[@]} -eq 0 ]; then
    check_result "治理 CI workflows" "PASS" "所有必需 workflows 存在"
else
    check_result "治理 CI workflows" "FAIL" "缺失 workflows: ${missing_workflows[*]}"
fi
...
governance_adrs=$(find "$GOVERNANCE_DIR" -name "ADR-*.md" -not -name "ADR-0000-*.md" -not -name "README.md")

adrs_without_adr0000_dep=()
for adr in $governance_adrs; do
    # 检查是否在 Relationships 章节依赖 ADR-0000
    if ! grep -A 20 "## 关系声明\|## Relationships" "$adr" | grep -q "ADR-0000"; then
...
if [ ${#adrs_without_adr0000_dep[@]} -eq 0 ]; then
    check_result "治理 ADR 依赖 ADR-0000" "PASS" "所有治理 ADR 正确声明依赖关系"
else
    check_result "治理 ADR 依赖 ADR-0000" "FAIL" "以下 ADR 未声明依赖 ADR-0000: ${adrs_without_adr0000_dep[*]}"
fi
...
readmes=$(find . -name "README.md" -not -path "*/node_modules/*" -not -path "*/.git/*" -not -path "*/bin/*" -not -path "*/obj/*")
...
if [ ${#readmes_without_disclaimer[@]} -eq 0 ]; then
    check_result "README 无裁决力声明" "PASS" "所有 README 包含权威边界声明"
else
    # 只报告文档目录下的 README
    doc_readmes=()
    ...
    if [ ${#doc_readmes[@]} -eq 0 ]; then
        check_result "README 无裁决力声明" "PASS" "文档 README 包含权威边界声明"
    else
        check_result "README 无裁决力声明" "FAIL" "以下文档 README 缺少声明: ${doc_readmes[*]}"
    fi
fi
...
if [ ${#adrs_without_change_policy[@]} -eq 0 ]; then
    check_result "治理 ADR 变更政策" "PASS" "核心治理 ADR 包含变更政策"
else
    check_result "治理 ADR 变更政策" "WARN" "以下 ADR 可能缺少变更政策说明: ${adrs_without_change_policy[*]}"
fi
...
if [ -d "src/tests/ArchitectureTests/ADR" ]; then
    ...
    if ls src/tests/ArchitectureTests/ADR/*0000* 1> /dev/null 2>&1; then
        check_result "ADR-0000 测试映射" "PASS" "存在 ADR-0000 相关测试"
    else
        check_result "ADR-0000 测试映射" "WARN" "未找到 ADR-0000 专门测试（可能集成在其他测试中）"
    fi
else
    check_result "架构测试目录" "FAIL" "架构测试目录不存在"
fi
