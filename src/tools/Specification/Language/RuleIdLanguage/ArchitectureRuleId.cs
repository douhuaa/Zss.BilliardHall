namespace Zss.BilliardHall.Specification.Language.RuleIdLanguage;
...
/// - 类型安全的构造
/// - 规范的字符串格式（ADR-907.3 或 ADR-907.3.2）
/// - 可比较、可排序
/// - 可用于测试失败信息
/// - 可映射到文档路径
/// </summary>
    ...
    : IComparable<ArchitectureRuleId>
{
    ...
    /// 
    /// 支持的格式（输入兼容）：
    /// - "ADR-907.3" → Rule 级别（推荐格式）
    /// - "ADR-907.3.2" → Clause 级别（推荐格式）
    /// - "ADR-907_3" → Rule 级别（兼容旧格式）
    /// - "ADR-907_3_2" → Clause 级别（兼容旧格式）
    /// 
    /// 注意：无论输入格式如何，ToString() 总是输出点号格式。
    /// 
