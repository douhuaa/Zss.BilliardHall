...
public sealed class AdrDecisionGenerator : IAdrDecisionGenerator
{
    ...
    /// </summary>
    public DecisionGenerationResult GenerateDecisionSection(IRuleSet ruleSet)
    {
    ...
    /// </summary>
    public DecisionGenerationResult GenerateDecisionSection(IRuleSet ruleSet, DecisionGenerationOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(ruleSet);
        ...
        // 统一行尾为 LF，避免跨平台差异
        var content = NormalizeNewlines(sb.ToString());
        return new DecisionGenerationResult(content);
    }
    ...
    /// 构建所有规则内容
    /// </summary>
    private static void BuildRulesContent(StringBuilder sb, IRuleSet ruleSet, DecisionGenerationOptions options)
    {
        ArgumentNullException.ThrowIfNull(sb);
        ArgumentNullException.ThrowIfNull(ruleSet);
        ArgumentNullException.ThrowIfNull(options);
    ...
    /// 获取排序后的规则列表
    /// </summary>
    private static List<IRule> GetOrderedRules(IRuleSet ruleSet)
    {
        ArgumentNullException.ThrowIfNull(ruleSet);
    ...
    /// 构建规则标题
    /// </summary>
    private static void BuildRuleHeader(StringBuilder sb, IRule rule, DecisionGenerationOptions options)
    {
        ArgumentNullException.ThrowIfNull(sb);
