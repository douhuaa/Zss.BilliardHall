...
1. **`generate adr`** - 生成 ADR Decision 章节并合并到现有文档
2. **`generate agent`** - 生成 Agent Instructions YAML 文件
3. **`generate test`** - 生成架构测试代码（C# xUnit）
4. **`validate`** - 校验 RuleSetRegistry 注册完整性与 RuleId 格式
...
### 3. `generate test` - 生成架构测试代码

根据 RuleSet 生成 C# xUnit 架构测试代码，使用 Theory + MemberData 模式。

**用法**：
```bash
dotnet run --project src/tools/Governance.Cli -- generate test --out <dir> [--adr <number>]
```

**参数**：
- `--out, -o`：输出目录【必需】
- `--adr, -a`：可选，仅生成指定 ADR 的测试代码（默认：全部）【可选】
- `--dry-run, -d`：Dry-run 模式，仅输出预览【可选】

**示例**：
```bash
# 生成所有 ADR 的架构测试代码
dotnet run --project src/tools/Governance.Cli -- generate test \
  --out src/tests/ArchitectureTests/Generated
...
**输出约定**：
- 文件命名：`Adr{number:D3}AutoGenerated_Tests.cs`（如：`Adr001AutoGenerated_Tests.cs`）
- 测试类命名空间：`Zss.BilliardHall.Tests.ArchitectureTests.Generated`
- 使用 xUnit Theory + MemberData 模式
- 每个 Clause 生成一个测试数据项
- 包含详细注释和使用说明

---

### 4. `validate` - 校验 RuleSet 注册完整性
