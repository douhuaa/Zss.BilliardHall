...
public sealed class ADR_004_2_Architecture_Tests
{
    ...
    public void ADR_004_2_1_Projects_Should_Not_Specify_Package_Versions()
    {
        ...
        (violations.Count == 0).Should().BeTrue(Build(
            ruleId: "ADR-004_2_1",
            violation: $"发现 {violations.Count} 个项目手动指定了包版本，应使用 CPM 统一管理",
            evidence: violations,
            remediation: new[]
            {
            ...
            reference: "docs/adr/constitutional/ADR-004-Cpm-Final.md §ADR-004_2_1"));
    }
    ...
    public void ADR_004_2_2_Directory_Packages_Props_Should_Define_All_Used_Packages()
    {
        ...
        (missingPackages.Count == 0).Should().BeTrue(Build(
            ruleId: "ADR-004_2_2",
            violation: $"发现 {missingPackages.Count} 个包在项目中使用但未在 Directory.Packages.props 中定义",
            evidence: new[] { $"缺失的包: {string.Join(", ", missingPackages)}" },
            remediation: new[]
            {
            ...
            reference: "docs/adr/constitutional/ADR-004-Cpm-Final.md §ADR-004_2_2"));
    }
}
