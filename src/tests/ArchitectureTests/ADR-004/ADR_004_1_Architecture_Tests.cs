...
public sealed class ADR_004_1_Architecture_Tests
{
    ...
    public void ADR_004_1_1_Repository_Should_Have_Directory_Packages_Props()
    {
        ...
        File.Exists(cpmFile).Should().BeTrue(BuildSimple(
            ruleId: "ADR-004_1_1",
            violation: "仓库根目录必须存在 Directory.Packages.props 文件以启用 Central Package Management (CPM)",
            detail: $"预期路径: {cpmFile}",
            remediation: "1. 在仓库根目录创建 Directory.Packages.props 文件\n" +
            ...
            reference: "docs/adr/constitutional/ADR-004-Cpm-Final.md §ADR-004_1_1"));
    }
    ...
    public void ADR_004_1_2_CPM_Should_Be_Enabled()
    {
        ...
        content.Contains("ManagePackageVersionsCentrally").Should().BeTrue(BuildSimple(
            ruleId: "ADR-004_1_2",
            violation: "Directory.Packages.props 必须包含 ManagePackageVersionsCentrally 设置",
            detail: "当前状态: 未找到 ManagePackageVersionsCentrally 配置",
            remediation: "1. 在 Directory.Packages.props 中添加 <PropertyGroup> 节点\n" +
                         "2. 添加 <ManagePackageVersionsCentrally>true</ManagePackageVersionsCentrally>\n" +
                         "3. 重新构建项目验证配置生效",
            reference: "docs/adr/constitutional/ADR-004-Cpm-Final.md §ADR-004_1_2"));

        content.Contains("true").Should().BeTrue(BuildSimple(
            ruleId: "ADR-004_1_2",
            violation: "Directory.Packages.props 中的 ManagePackageVersionsCentrally 应该设置为 true",
            detail: "当前状态: ManagePackageVersionsCentrally 值不正确",
            remediation: "1. 确保 <ManagePackageVersionsCentrally>true</ManagePackageVersionsCentrally>\n" +
                         "2. 检查拼写和大小写是否正确\n" +
    ...
    public void ADR_004_1_3_CPM_Should_Enable_Transitive_Pinning()
    {
        ...
        {
            content.Contains("<CentralPackageTransitivePinningEnabled>true</CentralPackageTransitivePinningEnabled>").Should().BeTrue(BuildSimple(
                ruleId: "ADR-004_1_3",
                violation: "建议启用 CentralPackageTransitivePinningEnabled 以固定传递依赖版本",
                detail: "当前状态: CentralPackageTransitivePinningEnabled 未设置为 true",
                remediation: "1. 在 Directory.Packages.props 中添加 <CentralPackageTransitivePinningEnabled>true</CentralPackageTransitivePinningEnabled>\n" +
                             "2. 这将确保所有传递依赖使用 CPM 中定义的版本\n" +
                             "3. 避免间接依赖升级导致的破坏性变更",
                reference: "docs/adr/constitutional/ADR-004-Cpm-Final.md §ADR-004_1_3",
                level: "建议"));
        }
    }
}
