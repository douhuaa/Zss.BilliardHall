namespace Zss.BilliardHall.Tests.ArchitectureTests.Specification.RuleSets.ADR946;

/// <summary>
/// ADR-946：安全扫描与审计规范
/// 定义治理层相关规则和约束（此为框架性定义，需根据实际测试文件完善）
/// </summary>
public sealed class Adr946RuleSet : IArchitectureRuleSetDefinition
{
    /// <summary>
    /// ADR 编号
    /// </summary>
    public int AdrNumber => 946;
    ...
    {
        var ruleSet = new ArchitectureRuleSet(946);

        // Rule 1: 核心规则（需根据测试文件完善）
        ruleSet.AddRule(
            ruleNumber: 1,
            summary: "安全扫描与审计规范核心规则",
