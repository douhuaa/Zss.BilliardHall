namespace Zss.BilliardHall.Tests.ArchitectureTests.Specification.RuleSets.ADR940;

/// <summary>
/// ADR-940：版本管理与发布策略
/// 定义治理层相关规则和约束（此为框架性定义，需根据实际测试文件完善）
/// </summary>
public sealed class Adr940RuleSet : IArchitectureRuleSetDefinition
{
    /// <summary>
    /// ADR 编号
    /// </summary>
    public int AdrNumber => 940;
    ...
    {
        var ruleSet = new ArchitectureRuleSet(940);

        // Rule 1: 核心规则（需根据测试文件完善）
        ruleSet.AddRule(
            ruleNumber: 1,
            summary: "版本管理与发布策略核心规则",
