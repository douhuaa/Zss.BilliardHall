namespace Zss.BilliardHall.Tests.ArchitectureTests.Specification.RuleSets.ADR930;

/// <summary>
/// ADR-930：持续集成流程规范
/// 定义治理层相关规则和约束（此为框架性定义，需根据实际测试文件完善）
/// </summary>
public sealed class Adr930RuleSet : IArchitectureRuleSetDefinition
{
    /// <summary>
    /// ADR 编号
    /// </summary>
    public int AdrNumber => 930;
    ...
    {
        var ruleSet = new ArchitectureRuleSet(930);

        // Rule 1: 核心规则（需根据测试文件完善）
        ruleSet.AddRule(
            ruleNumber: 1,
            summary: "持续集成流程规范核心规则",
