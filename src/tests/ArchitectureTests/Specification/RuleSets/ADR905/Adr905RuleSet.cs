namespace Zss.BilliardHall.Tests.ArchitectureTests.Specification.RuleSets.ADR905;

/// <summary>
/// ADR-905：执行级别分类体系
/// 定义治理层相关规则和约束（此为框架性定义，需根据实际测试文件完善）
/// </summary>
public sealed class Adr905RuleSet : IArchitectureRuleSetDefinition
{
    /// <summary>
    /// ADR 编号
    /// </summary>
    public int AdrNumber => 905;
    ...
    {
        var ruleSet = new ArchitectureRuleSet(905);

        // Rule 1: 核心规则（需根据测试文件完善）
        ruleSet.AddRule(
            ruleNumber: 1,
            summary: "执行级别分类体系核心规则",
