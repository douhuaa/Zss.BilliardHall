...
public sealed class ADR_005_1_Architecture_Tests
{
    ...
    public void ADR_005_1_1_Each_Use_Case_Must_Have_Unique_Handler(Assembly moduleAssembly)
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            (isCommandHandler || isQueryHandler || isEventHandler).Should().BeTrue(Build(
                ruleId: "ADR-005_1_1",
                violation: "Handler 命名不清晰",
                evidence: new[] { $"违规类型: {handler.FullName}" },
                analysis: "Handler 命名未明确表达业务意图（Command/Query/Event）",
                remediation: new[]
                {
                ...
                reference: "ADR-005_1_1 - 每个业务用例必须唯一 Handler"));
        }
    }
        ...
        foreach (var endpoint in endpoints)
        {
            ...
            if (constructorParams.Count > 5)
            {
                true.Should().BeFalse(Build(
                    ruleId: "ADR-005_1_2",
                    violation: "Endpoint/Controller 包含过多依赖",
                    evidence: new[]
                    {
                        $"违规类型: {endpoint.FullName}",
                        $"构造函数依赖数量: {constructorParams.Count} 个（超过建议的 5 个）"
                    },
                    analysis: "Endpoint/Controller 注入过多业务依赖，可能包含业务逻辑",
                    remediation: new[]
                    {
                    ...
                    reference: "ADR-005_1_2 - Endpoint 仅做请求适配"));
            }
        }
    }
}
