...
/// 
/// 测试映射：
/// - All_ADR_Files_Should_Match_Numbering_Rules → ADR-0006.1 (ADR 文件编号应符合分段规则)
/// - All_ADR_Files_Should_Have_Correct_Filename_Format → ADR-0006.2 (ADR 文件名应与编号一致)
/// - Constitutional_ADRs_Should_Use_Leading_Zeros → ADR-0006.3 (宪法层 ADR 应使用前导零)
/// - Non_Constitutional_ADRs_Should_Not_Use_Leading_Zeros → ADR-0006.4 (非宪法层 ADR 不应使用前导零)
/// - All_ADR_Files_Should_Be_In_Correct_Directory → ADR-0006.5 (ADR 文件应位于正确的目录)
/// </summary>
public sealed class ADR_0006_Architecture_Tests
{
    ...
    public void All_ADR_Files_Should_Match_Numbering_Rules()
    {
        ...
        foreach (var file in adrFiles)
        {
            ...
            Assert.True(match.Success,
                ...
                $"3. 其他层（100+）不使用前导零\n\n" +
                $"参考: docs/copilot/adr-0006.prompts.md (场景 5)");
    ...
    public void All_ADR_Files_Should_Have_Correct_Filename_Format()
    {
        ...
        foreach (var file in adrFiles)
        {
            ...
            Assert.True(isValid,
                ...
                $"3. 标题应简洁但具描述性\n\n" +
                $"说明:\n" +
                $"历史文件（如 ADR-0004-Cpm-Final.md）已被豁免，但新 ADR 必须遵循规范\n\n" +
                $"参考: docs/copilot/adr-0006.prompts.md (场景 5)");
        }
    }
            ...
            if (match.Success)
            {
                ...
                if (number is >= 1 and <= 9)
                {
                    Assert.True(numberStr.Length == 4 && numberStr.StartsWith("000"),
                        ...
                        $"2. 文件名格式: ADR-0001, ADR-0002, ..., ADR-0009\n\n" +
                        $"参考: docs/copilot/adr-0006.prompts.md (常见错误 2)");
                }
            }
        }
    }
            ...
            foreach (var file in adrFiles)
            {
                ...
                if (match.Success)
                {
                    ...
                    Assert.True(isInCorrectRange,
                        ...
                        $"- 200~299: runtime/\n" +
                        $"- 300~399: technical/\n\n" +
                        $"参考: docs/copilot/adr-0006.prompts.md (场景 3)");
                }
            }
        }
    }
