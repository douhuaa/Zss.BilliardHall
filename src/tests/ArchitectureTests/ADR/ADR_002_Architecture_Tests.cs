...
/// 
/// 测试映射：
/// - Platform_Should_Not_Depend_On_Application → ADR-002.1 (Platform 不应依赖 Application)
/// - Platform_Should_Not_Depend_On_Host → ADR-002.2 (Platform 不应依赖 Host)
/// - Platform_Should_Not_Depend_On_Modules → ADR-002.3 (Platform 不应依赖任何 Modules)
/// - Platform_Should_Have_Single_Bootstrapper_Entry_Point → ADR-002.4 (Platform 应有唯一的 PlatformBootstrapper 入口)
/// - Application_Should_Not_Depend_On_Host → ADR-002.5 (Application 不应依赖 Host)
/// - Application_Should_Not_Depend_On_Modules → ADR-002.6 (Application 不应依赖任何 Modules)
/// - Application_Should_Have_Single_Bootstrapper_Entry_Point → ADR-002.7 (Application 应有唯一的 ApplicationBootstrapper 入口)
/// - Application_Should_Not_Use_HttpContext → ADR-002.8 (Application 不应包含 HttpContext 等 Host 专属类型)
/// - Host_Should_Not_Depend_On_Modules → ADR-002.9 (Host 不应依赖任何 Modules)
/// - Host_Should_Not_Contain_Business_Types → ADR-002.10 (Host 不应包含业务类型)
/// - Host_Csproj_Should_Not_Reference_Modules → ADR-002.11 (Host 项目文件不应引用 Modules)
/// - Program_Cs_Should_Be_Concise → ADR-002.12 (Program.cs 应保持简洁)
/// - Program_Cs_Should_Only_Call_Bootstrapper → ADR-002.13 (Program.cs 只应调用 Bootstrapper)
/// - Verify_Complete_Three_Layer_Dependency_Direction → ADR-002.14 (验证完整的三层依赖方向)
/// </summary>
public sealed class ADR_002_Architecture_Tests
{
    ...
    public void Platform_Should_Not_Depend_On_Application()
    {
        ...
        $"参考: docs/copilot/adr-002.prompts.md (场景 2)");
    }
    ...
    public void Platform_Should_Have_Single_Bootstrapper_Entry_Point()
    {
        ...
        configureMethods.Should().NotBeEmpty($"❌ ADR-002_4_1 违规: PlatformBootstrapper 必须包含 Configure 方法\n\n" +
        $"修复建议:\n" +
    ...
    public void Application_Should_Not_Depend_On_Host()
    {
        ...
        $"参考: docs/copilot/adr-002.prompts.md (场景 3)");
    }
    ...
    public void Application_Should_Have_Single_Bootstrapper_Entry_Point()
    {
        ...
        configureMethods.Should().NotBeEmpty($"❌ ADR-002_1_7 违规: ApplicationBootstrapper 必须包含 Configure 方法\n\n" +
        $"修复建议:\n" +
    ...
    public void Application_Should_Not_Use_HttpContext()
    {
        ...
        $"参考: docs/copilot/adr-002.prompts.md (场景 3, 反模式 3)");
    }
    ...
    public void Host_Should_Not_Depend_On_Modules(Assembly hostAssembly)
    {
        ...
        $"参考: docs/copilot/adr-002.prompts.md (场景 1, 反模式 4)");
    }
    ...
    public void Program_Cs_Should_Be_Concise()
    {
        ...
        foreach (var programFile in programFiles)
        {
            ...
            $"参考: docs/copilot/adr-002.prompts.md (场景 4)");
        }
    }
