...
/// 
/// 测试映射：
/// - All_Types_Should_Start_With_Base_Namespace → ADR-003.1 (所有类型应以 BaseNamespace 开头)
/// - Platform_Types_Should_Have_Platform_Namespace → ADR-003.2 (Platform 类型应在 Zss.BilliardHall.Platform 命名空间)
/// - Application_Types_Should_Have_Application_Namespace → ADR-003.3 (Application 类型应在 Zss.BilliardHall.Application 命名空间)
/// - Module_Types_Should_Have_Module_Namespace → ADR-003.4 (Module 类型应在 Zss.BilliardHall.Modules.{ModuleName} 命名空间)
/// - Host_Types_Should_Have_Host_Namespace → ADR-003.5 (Host 类型应在 Zss.BilliardHall.Host.{HostName} 命名空间)
/// - Directory_Build_Props_Should_Exist_At_Repository_Root → ADR-003.6 (Directory.Build.props 应存在于仓库根目录)
/// - Directory_Build_Props_Should_Define_Base_Namespace → ADR-003.7 (Directory.Build.props 应定义 BaseNamespace)
/// - All_Projects_Should_Follow_Namespace_Convention → ADR-003.8 (所有项目应遵循命名空间约定)
/// - Modules_Should_Not_Contain_Irregular_Namespace_Patterns → ADR-003.9 (模块不应包含不规范的命名空间模式)
/// </summary>
public sealed class ADR_003_Architecture_Tests
{
    ...
    public void Directory_Build_Props_Should_Define_Base_Namespace()
    {
        ...
        {
            true.Should().BeFalse($"❌ ADR-003_6_1 违规: Directory.Build.props 文件不存在");
        }

        var content = File.ReadAllText(directoryBuildPropsPath);

        (content.Contains("CompanyNamespace") || content.Contains("ProductNamespace") || content.Contains("BaseNamespace")).Should().BeTrue($"❌ ADR-003_7_1 违规: Directory.Build.props 应定义 BaseNamespace 相关属性\n\n" +
