...
/// ├─────────────┼────────────────────────────────────────────────────────┼──────────┤
/// │ ADR-005.1  │ Handler 应有明确的命名约定                              │ 1, 8     │
/// │ ADR-005.2  │ Endpoint 不应包含业务逻辑                               │ 1, 8     │
/// │ ADR-005.3  │ Handler 不应依赖 ASP.NET 类型                           │ 2, 8     │
/// │ ADR-005.4  │ Handler 应该是无状态的                                  │ 2, 8     │
/// │ ADR-005.5  │ 模块间不应有未审批的同步调用                            │ 3, 8     │
/// │ ADR-005.6  │ 异步方法应遵循命名约定                                  │ 3, 8     │
/// │ ADR-005.7  │ 模块不应共享领域实体                                    │ 4, 8     │
/// │ ADR-005.8  │ Query Handler 可以返回 Contracts                       │ 4, 5, 8  │
/// │ ADR-005.9  │ Command Handler 和 Query Handler 应明确分离            │ 5, 8     │
/// │ ADR-005.10 │ Command Handler 不应返回业务数据                        │ 5, 8     │
/// │ ADR-005.11 │ Handler 应使用结构化异常                                │ 7, 8     │
/// │ ADR-005.12 │ 所有 Handler 应在模块程序集中                           │ 2, 8     │
/// └─────────────┴────────────────────────────────────────────────────────┴──────────┘
/// </summary>
public sealed class ADR_005_Architecture_Tests
{
    ...
    [Theory(DisplayName = "ADR-005_1_1: Handler 应有明确的命名约定")]
    ...
    public void Handlers_Should_Have_Clear_Naming_Convention(Assembly moduleAssembly)
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            $"参考: docs/copilot/adr-005.prompts.md（场景 1-3）");
        }
    }

    [Theory(DisplayName = "ADR-005_1_2: Endpoint 不应包含业务逻辑")]
    [ClassData(typeof(ModuleAssemblyData))]
            ...
            foreach (var method in methods)
            {
                ...
                if (constructorParams.Count > 5)
                {
                    true.Should().BeFalse($"❌ ADR-005_1_2 违规: Endpoint/Controller 包含过多依赖\n\n" +
                                ...
                                $"参考: docs/copilot/adr-005.prompts.md（场景 5，反模式 1）");
                }
            }
        }
    }
    ...
    [Theory(DisplayName = "ADR-005_2_1: Handler 不应依赖 ASP.NET 类型")]
    [ClassData(typeof(ModuleAssemblyData))]
    public void Handlers_Should_Not_Depend_On_AspNet(Assembly moduleAssembly)
    {
        ...
        result.IsSuccessful.Should().BeTrue($"❌ ADR-005_3_1 违规: Handler 依赖 ASP.NET Core 类型\n\n" +
        ...
        $"参考: docs/copilot/adr-005.prompts.md（场景 2）");
        ...
        foreach (var handler in handlers)
        {
            ...
            (ctorDeps.Count == 0).Should().BeTrue($"❌ ADR-005_3_1 违规: Handler 构造函数依赖 ASP.NET 类型\n\n" +
    ...
    [Theory(DisplayName = "ADR-005_2_2: Handler 应该是无状态的")]
    [ClassData(typeof(ModuleAssemblyData))]
    public void Handlers_Should_Be_Stateless(Assembly moduleAssembly)
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            (fields.Count == 0).Should().BeTrue($"❌ ADR-005_2_2 违规: Handler 包含可变字段\n\n" +
            $"违规 Handler: {handler.FullName}\n" +
            ...
            $"参考: docs/copilot/adr-005.prompts.md（反模式 3）");
        }
    }
    ...
    [Theory(DisplayName = "ADR-005_3_1: 模块间不应有未审批的同步调用")]
    [ClassData(typeof(ModuleAssemblyData))]
        ...
        foreach (var handler in handlers)
        {
            ...
            foreach (var param in ctorParams)
            {
                ...
                if (param.Namespace != null && param.Namespace.StartsWith("Zss.BilliardHall.Modules"))
                {
                    ...
                    {
                        true.Should().BeFalse($"❌ ADR-005_3_1 违规: Handler 注入了其他模块的类型\n\n" +
                                    $"违规 Handler: {handler.FullName}\n" +
                                    $"当前模块: {currentModule}\n" +
                                    $"依赖模块: {paramModule}\n" +
                                    $"依赖类型: {param.FullName}\n\n" +
                                    ...
                                    $"参考: docs/copilot/adr-005.prompts.md（场景 4，反模式 4）");
                    }
                }
            }
        }
    ...
    [Theory(DisplayName = "ADR-005_3_2: 异步方法应遵循命名约定")]
    [ClassData(typeof(ModuleAssemblyData))]
    public void Async_Methods_Should_Follow_Naming_Convention(Assembly moduleAssembly)
    {
        ...
        foreach (var type in types)
        {
            ...
            foreach (var method in methods)
            {
                ...
                {
                    true.Should().BeFalse($"❌ ADR-005_3_2 违规: 异步方法命名不符合约定\n\n" +
                                ...
                                $"参考: docs/copilot/adr-005.prompts.md（场景 2-3）");
                }
            }
        }
    }
    ...
    [Theory(DisplayName = "ADR-005_4_1: 模块不应共享领域实体")]
    [ClassData(typeof(ModuleAssemblyData))]
    public void Modules_Should_Not_Share_Domain_Entities(Assembly moduleAssembly)
    {
        ...
        foreach (var entity in entities)
        {
            ...
            if (entity.IsPublic)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine($"⚠️ ADR-005 建议: 实体 {entity.FullName} 是 public 且有公共构造函数，确保不会被其他模块直接引用。");
                }
            }
        }
    ...
    [Theory(DisplayName = "ADR-005_4_2: Query Handler 可以返回 Contracts")]
    [ClassData(typeof(ModuleAssemblyData))]
    ...
    [Theory(DisplayName = "ADR-005_5_1: Command Handler 和 Query Handler 应明确分离")]
    [ClassData(typeof(ModuleAssemblyData))]
    public void Command_And_Query_Handlers_Should_Be_Separated(Assembly moduleAssembly)
    {
        ...
        foreach (var handler in handlers)
        {
            ...
            {
                true.Should().BeFalse($"❌ ADR-005_5_1 违规: Handler 同时包含 Command 和 Query 语义\n\n" +
                            $"违规 Handler: {handler.FullName}\n\n" +
                            $"问题分析:\n" +
                            ...
                            $"参考: docs/copilot/adr-005.prompts.md（FAQ Q1）");
            }
        }
    }

    [Theory(DisplayName = "ADR-005_5_2: Command Handler 不应返回业务数据")]
    [ClassData(typeof(ModuleAssemblyData))]
        ...
        foreach (var handler in commandHandlers)
        {
            ...
            foreach (var method in handleMethods)
            {
                ...
                if (!allowedTypes.Contains(returnType.Name) && returnType != typeof(void))
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"⚠️ ADR-005 建议: Command Handler {handler.FullName}.{method.Name} 返回了业务数据类型 {returnType.FullName}。\n" + $"建议：Command Handler 应该只返回简单类型（如 ID/bool）或 void，避免返回完整的业务对象。");
                    }
                }
            }
        }
    ...
    [Theory(DisplayName = "ADR-005_6_1: Handler 应使用结构化异常")]
    [ClassData(typeof(ModuleAssemblyData))]
    ...
    [Fact(DisplayName = "ADR-005_7_1: 所有 Handler 应在模块程序集中")]
