...
/// 
/// 测试-ADR 映射清单：
/// ├─ ADR-004.1: Repository_Should_Have_Directory_Packages_Props → Directory.Packages.props 必须存在
/// ├─ ADR-004.2: CPM_Should_Be_Enabled → ManagePackageVersionsCentrally 必须启用
/// ├─ ADR-004.3: CPM_Should_Enable_Transitive_Pinning → CentralPackageTransitivePinningEnabled 建议启用
/// ├─ ADR-004.4: Projects_Should_Not_Specify_Package_Versions → 项目文件不应手动指定包版本
/// ├─ ADR-004.5: Directory_Packages_Props_Should_Contain_Package_Groups → 包应按功能分组
/// ├─ ADR-004.6: Directory_Packages_Props_Should_Contain_Common_Package_Groups → 应包含常见分组
/// ├─ ADR-004.7: Platform_Projects_Should_Not_Reference_Business_Packages → Platform 不引用业务包
/// ├─ ADR-004.8: All_Test_Projects_Should_Use_Same_Test_Framework_Versions → 测试框架版本统一
/// └─ ADR-004.9: Directory_Packages_Props_Should_Define_All_Used_Packages → CPM 定义所有使用的包
/// 
/// 参考文档：
/// - ADR-004: docs/adr/constitutional/ADR-004-Cpm-Final.md
/// - Copilot Prompts: docs/copilot/adr-004.prompts.md
/// </summary>
public sealed class ADR_004_Architecture_Tests
{
    ...
    public void Repository_Should_Have_Directory_Packages_Props()
    {
        ...
        File.Exists(cpmFile).Should().BeTrue($"❌ ADR-004_1_1 违规: 仓库根目录必须存在 Directory.Packages.props 文件以启用 Central Package Management (CPM)。\n\n" +
        $"预期路径: {cpmFile}\n\n" +
        $"修复建议:\n" +
        ...
        $"参考: docs/copilot/adr-004.prompts.md (场景 1)");
    }
    ...
    public void CPM_Should_Be_Enabled()
    {
        ...
        content.Contains("ManagePackageVersionsCentrally").Should().BeTrue($"❌ ADR-004_2_1 违规: Directory.Packages.props 必须包含 ManagePackageVersionsCentrally 设置。\n\n" +
        ...
        content.Contains("true").Should().BeTrue($"❌ ADR-004_2_1 违规: Directory.Packages.props 中的 ManagePackageVersionsCentrally 应该设置为 true。\n\n" +
    ...
    public void CPM_Should_Enable_Transitive_Pinning()
    {
        ...
        {
            content.Contains("<CentralPackageTransitivePinningEnabled>true</CentralPackageTransitivePinningEnabled>").Should().BeTrue($"⚠️ ADR-004.3 建议: 建议启用 CentralPackageTransitivePinningEnabled 以固定传递依赖版本。\n\n" +
            ...
            $"3. 避免间接依赖升级导致的破坏性变更\n\n" +
            $"参考: docs/copilot/adr-004.prompts.md (场景 4)");
        }
    }
    ...
    public void Projects_Should_Not_Specify_Package_Versions()
    {
        ...
        (violations.Count == 0).Should().BeTrue($"❌ ADR-004_2_1 违规: 发现 {violations.Count} 个项目手动指定了包版本，应使用 CPM 统一管理。\n\n" +
        $"违规项目:\n{string.Join("\n", violations)}\n\n" +
        $"修复建议:\n" +
        ...
        $"参考: docs/copilot/adr-004.prompts.md (场景 1, 反模式 1)");
    }
    ...
    public void Directory_Packages_Props_Should_Contain_Package_Groups()
    {
        ...
        (itemGroups != null && itemGroups.Count > 0).Should().BeTrue($"⚠️ ADR-004.5 建议: 建议在 Directory.Packages.props 中使用 Label 属性对包进行分组。\n\n" +
        $"当前状态: 未发现使用 Label 属性的包分组\n\n" +
        $"修复建议:\n" +
        $"1. 使用 <ItemGroup Label=\"分组名称\"> 对包进行逻辑分组\n" +
        $"2. 常见分组: Logging, Testing, Wolverine Framework, Marten, Aspire 等\n" +
        $"3. 这有助于快速定位和管理相关包\n\n" +
        $"参考: docs/copilot/adr-004.prompts.md (FAQ Q3)");
    }

    [Fact(DisplayName = "ADR-004_3_2: Directory.Packages.props 应包含常见包分组")]
    public void Directory_Packages_Props_Should_Contain_Common_Package_Groups()
    {
        ...
        {
            // 这只是建议，不强制失败
            System.Diagnostics.Debug.WriteLine($"⚠️ ADR-004 建议: 建议在 Directory.Packages.props 中添加以下分组：{string.Join(", ", missingGroups)}");
        }
    ...
    [Fact(DisplayName = "ADR-004_4_1: Platform 项目不应引用业务包")]
    public void Platform_Projects_Should_Not_Reference_Business_Packages()
    {
        ...
        foreach (var projectFile in platformProjects)
        {
            ...
            foreach (XmlNode packageRef in packageRefs)
            {
                ...
                {
                    true.Should().BeFalse($"❌ ADR-004_4_1 违规: Platform 项目 {Path.GetFileName(projectFile)} 不应引用业务包: {packageName}。\n\n" +
                                ...
                                $"参考: docs/copilot/adr-004.prompts.md (场景 3, 反模式 2)");
                }
            }
        }
    }

    [Fact(DisplayName = "ADR-004_4_2: 所有测试项目应引用相同的测试框架版本")]
    public void All_Test_Projects_Should_Use_Same_Test_Framework_Versions()
    {
        ...
        foreach (var kvp in packageVersions)
        {
            ...
            {
                true.Should().BeFalse($"❌ ADR-004_4_2 违规: 测试包 {kvp.Key} 存在多个版本: {string.Join(", ", kvp.Value)}。\n\n" +
                            $"检测到的版本: {string.Join(", ", kvp.Value)}\n\n" +
                            $"修复建议:\n" +
                            $"1. 在 Directory.Packages.props 中统一该包的版本\n" +
                            $"2. 从所有项目文件中移除手动指定的版本号\n" +
                            $"3. 确保所有测试项目使用相同的测试框架版本\n\n" +
                            $"参考: docs/copilot/adr-004.prompts.md (场景 1, FAQ Q2)");
            }
        }
    }
    ...
    [Fact(DisplayName = "ADR-004_5_1: Directory.Packages.props 应定义所有项目使用的包")]
    public void Directory_Packages_Props_Should_Define_All_Used_Packages()
    {
        ...
        (missingPackages.Count == 0).Should().BeTrue($"❌ ADR-004_5_1 违规: 发现 {missingPackages.Count} 个包在项目中使用但未在 Directory.Packages.props 中定义。\n\n" +
        ...
        $"参考: docs/copilot/adr-004.prompts.md (场景 1, FAQ Q1)");
    }
