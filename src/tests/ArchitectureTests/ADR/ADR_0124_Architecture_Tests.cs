...
{
    [Theory(DisplayName = "ADR-0124_1_1: Endpoint 类必须遵循命名规范")]
    ...
    [Theory(DisplayName = "ADR-0124_1_2: 请求 DTO 必须以 Request 结尾")]
    [ClassData(typeof(ModuleAssemblyData))]
    public void Request_DTOs_Must_End_With_Request(Assembly moduleAssembly)
    {
        ...
        foreach (var dto in requestDtos)
        {
            ...
            endsWithRequest.Should().BeTrue($"❌ ADR-0124_1_2 违规: 请求 DTO 命名不符合规范\n\n" +
                $"违规类型: {dto.FullName}\n\n" +
                $"修复建议:\n" +
