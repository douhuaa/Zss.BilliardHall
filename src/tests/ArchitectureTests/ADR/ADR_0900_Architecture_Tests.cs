...
public sealed class ADR_0900_Architecture_Tests
{
    ...
    public void ADR_Documents_Must_Have_Architecture_Tests_And_Prompts()
    {
        ...
        Assert.True(Directory.Exists(adrDirectory), 
            $"未找到 ADR 文档目录：{adrDirectory}");
    ...
    public void ADR_Documents_Must_Mark_Testable_Constraints()
    {
        ...
        foreach (var adrNumber in testFiles)
        {
            ...
            foreach (var adrFile in adrFiles)
            {
                ...
                // 检查是否有测试标记
                var hasTestMarker = contentWithoutCodeBlocks.Contains("【必须架构测试覆盖】") ||
                                   contentWithoutCodeBlocks.Contains("【必须测试】") ||
                                   contentWithoutCodeBlocks.Contains("[MUST_TEST]");
                
                // 检查是否有快速参考表
                var hasQuickReference = content.Contains("## 快速参考") || 
                                       content.Contains("##快速参考") ||
                                       content.Contains("## 快速参考表");
