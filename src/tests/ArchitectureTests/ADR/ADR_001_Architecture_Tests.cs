...
/// <summary>
/// ADR-001: 模块化单体与垂直切片架构决策（v4.0）
/// 验证模块隔离、垂直切片、契约使用等核心架构约束
/// 
/// 约束映射（对应 ADR-001 v4.0 快速参考和架构测试映射表）：
/// - ADR-001.1: 模块不可相互引用 (L1) → Modules_Should_Not_Reference_Other_Modules
/// - ADR-001.2: 项目文件/程序集禁止引用其他模块 (L1) → Module_Csproj_Should_Not_Reference_Other_Modules
/// - ADR-001.3: 垂直切片/用例为最小单元 (L2) → Handlers_Should_Be_In_UseCases_Namespace
/// - ADR-001.4: 禁止横向 Service 抽象 (L1) → Modules_Should_Not_Contain_Service_Classes
/// - ADR-001.5: 只允许事件/契约/原始类型通信 (L2) → Contract_Rules_Semantic_Check
/// - ADR-001.6: Contract 不含业务判断字段 (L2/L3) → Contract_Business_Field_Analyzer
/// - ADR-001.7: 命名空间/目录强制隔离 (L1) → Namespace_Should_Match_Module_Boundaries
/// </summary>
public sealed class ADR_001_Architecture_Tests
{

    #region 1. 模块隔离约束 (ADR-001.1, 0001.2, 0001.7)
    ...
    #region 2. 垂直切片架构约束 (ADR-001.3, 0001.4)
    ...
    #region 3. 契约使用规则约束 (ADR-001.5, 0001.6)
