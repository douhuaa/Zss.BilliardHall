...
/// 验证所有 Agent 配置文件符合 ADR-007 的行为约束和权限边界
///
/// 【测试覆盖映射】
/// ├─ ADR-007.1: 三态输出强制 → Agent_Responses_Must_Include_Three_State_Indicators
/// ├─ ADR-007.3: 禁止批准破例 → Agent_Files_Must_Prohibit_Architecture_Bypass
/// ├─ ADR-007.5: 裁决引用 ADR → Agent_Files_Must_Reference_ADR_As_Authority
/// ├─ ADR-007.7: Prompts 一致性 → Prompts_Must_Not_Contradict_ADR (警告级)
/// ├─ ADR-007.8: Guardian 关系 → Specialist_Agents_Must_Declare_Guardian_Relationship
/// └─ ADR-007.9: 版本历史 → Agent_Files_Must_Have_Version_History
///
/// 【关联文档】
/// - ADR: docs/adr/constitutional/ADR-007-agent-behavior-permissions-constitution.md
/// - Prompts: docs/copilot/adr-007.prompts.md
/// - Agents: .github/agents/
/// </summary>
public sealed class ADR_007_Architecture_Tests
{
    ...
    [Fact(DisplayName = "ADR-007_1_1: Agent 配置必须包含三态输出规范")]
    public void Agent_Responses_Must_Include_Three_State_Indicators()
    {
        ...
        if (violations.Any())
            true.Should().BeFalse(FormatViolations("ADR-007.1 违规：以下 Agent 配置文件未正确实现三态输出规范：", violations));
    }

    [Fact(DisplayName = "ADR-007_1_2: Prompts 文件不应引入 ADR 未明确的规则")]
    public void Prompts_Must_Not_Contradict_ADR()
    {
        ...
        var message = "\n⚠️ ADR-007.7 提醒：Prompts 文件可能需要人工审查：\n" + string.Join("\n", warnings);
