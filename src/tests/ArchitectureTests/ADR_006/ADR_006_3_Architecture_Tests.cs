        ...
        foreach (var file in adrFiles)
        {
            ...
            if (match.Success)
            {
                ...
                (numberStr.Length == 3).Should().BeTrue(Build(
                    ...
                    analysis: "编号小于100时必须使用前导零，确保统一的3位格式",
                    remediation: new[]
                    {
                    ...
                    reference: "ADR-006_3_1 - 前导零强制要求"));

                // Specifically check for numbers < 100
                if (number < 100)
                {
                    numberStr.Should().StartWith("0", BuildSimple(
                        ruleId: "ADR-006_3_1",
                        violation: "小于100的 ADR 编号必须使用前导零",
                        detail: $"文件: {file}\n当前编号: ADR-{numberStr}\n正确编号: ADR-{number:D3}",
                        remediation: "",
                        reference: "ADR-006_3_1 - 前导零强制要求"));
                }
