# Wolverine 项目测试入口说明

## 概述

本文档说明 Wolverine 项目的测试入口位置和运行方式。

**✨ 重要变更（2026-01-11）**：项目现采用**分层测试策略**，降低对 Aspire AppHost 的依赖，提高 CI 测试可靠性。

## 测试架构：三层验证体系

```
┌─────────────────────────────────────────────────┐
│  第一层：单元测试（Unit Tests）                   │
│  - ServiceDefaults 配置测试                      │
│  - Bootstrapper 烟雾测试                         │
│  - 无需外部依赖，快速反馈                          │
│  ✅ CI 每次 PR 运行                              │
└─────────────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────┐
│  第二层：轻量集成测试（Lightweight Integration）   │
│  - Bootstrapper + Testcontainers PostgreSQL     │
│  - Marten 文档存储集成                           │
│  - 仅需 Docker，无需 Aspire DCP                  │
│  ⏸️ 本地开发手动运行                              │
└─────────────────────────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────┐
│  第三层：端到端测试（E2E Tests）                  │
│  - AppHost + Aspire DCP + 完整编排               │
│  - 多服务协同验证                                 │
│  ❌ 仅在手动触发或 push main 时运行               │
└─────────────────────────────────────────────────┘
```

## 测试项目位置

当前 Wolverine 项目包含 **3 个测试项目**：

### 1. Zss.BilliardHall.Wolverine.ServiceDefaults.Tests ⚡ 第一层

**路径**: `src/Wolverine/Aspire/Zss.BilliardHall.Wolverine.ServiceDefaults.Tests/`

**项目文件**: `Zss.BilliardHall.Wolverine.ServiceDefaults.Tests.csproj`

**测试框架**: xUnit

**测试内容**: 
- ServiceDefaults 扩展方法集成测试
- Marten 配置测试
- OpenTelemetry 配置测试
- HealthChecks 配置测试
- Wolverine 默认配置测试

**包含的测试文件**:
1. `MartenExtensionsTests.cs` - Marten 扩展方法测试（7个测试用例）
2. `ServiceDefaultsIntegrationTests.cs` - ServiceDefaults 集成测试（12个测试用例）

**测试数量**：19 个

**特点**：
- ✅ 无需外部依赖
- ✅ 快速执行（< 5 秒）
- ✅ CI 每次 PR 运行

### 2. Zss.BilliardHall.Wolverine.Bootstrapper.Tests 🚀 第二层（新增）

**路径**: `src/Wolverine/Bootstrapper.Tests/`

**项目文件**: `Zss.BilliardHall.Wolverine.Bootstrapper.Tests.csproj`

**测试框架**: xUnit + Testcontainers

**测试内容**:
- **烟雾测试（Smoke Tests）**：验证 Bootstrapper 配置完整性
- **集成测试（Integration Tests）**：使用真实 PostgreSQL 容器验证 Marten + Wolverine

**包含的测试文件**:
1. `BootstrapperSmokeTests.cs` - 烟雾测试（6个，`Category=Unit`）
2. `BootstrapperIntegrationTests.cs` - 集成测试（5个，`Category=Integration+RequiresDocker`）
3. `PostgresFixture.cs` - PostgreSQL 测试容器管理

**测试数量**：11 个
- 烟雾测试（Unit）：6 个
- 集成测试（Integration）：5 个

**特点**：
- ✅ 烟雾测试无需 Docker，CI 每次 PR 运行
- 🐳 集成测试需要 Docker（本地手动运行）
- 🚀 使用 Testcontainers，无需 Aspire DCP
- 🔄 自动管理容器生命周期

**详细文档**: [Bootstrapper.Tests README](Bootstrapper.Tests/README.md)

### 3. Zss.BilliardHall.Wolverine.AppHost.Tests 🏗️ 第三层

**路径**: `src/Wolverine/Aspire/Zss.BilliardHall.Wolverine.AppHost.Tests/`

**项目文件**: `Zss.BilliardHall.Wolverine.AppHost.Tests.csproj`

**测试框架**: xUnit + Aspire.Hosting.Testing

**测试内容**:
- AppHost 编排验证
- 多服务协同测试
- 完整端到端验证

**包含的测试文件**:
1. `AppHostIntegrationTests.cs` - AppHost 集成测试（3个测试，默认 Skip）

**测试数量**：3 个（默认被跳过）

**特点**：
- ⚠️ 需要 Docker + Aspire DCP
- ⏱️ 启动慢（容器编排）
- ❌ CI 中不自动运行（仅手动触发或 push main）
- 🔒 默认 Skip，需手动启用

**详细文档**: [AppHost.Tests README](Aspire/Zss.BilliardHall.Wolverine.AppHost.Tests/README.md)

## 测试用例列表

### MartenExtensionsTests (7个测试)
1. `AddMartenDefaults_WithValidConnectionString_ShouldRegisterMartenServices` - 验证使用有效连接字符串注册 Marten 服务
2. `AddMartenDefaults_WithMissingConnectionString_ShouldThrowInvalidOperationException` - 验证缺少连接字符串时抛出异常
3. `AddMartenDefaults_ShouldConfigureSchemaName` - 验证配置 Schema 名称为 "billiard"
4. `AddMartenDefaults_ShouldUseProvidedConnectionString` - 验证使用提供的连接字符串
5. `AddMartenDefaults_WithValidConfiguration_ShouldSucceed` - 验证有效配置成功
6. `AddMartenDefaults_WithEmptyConnectionString_ShouldThrowInvalidOperationException` - 验证空连接字符串时抛出异常
7. `AddMartenDefaults_ShouldConfigureLightweightSessions` - 验证配置轻量级会话

### ServiceDefaultsIntegrationTests (12个测试)
1. `AddServiceDefaults_ShouldRegisterAllRequiredServices` - 验证注册所有必需服务
2. `AddDefaultHealthChecks_ShouldRegisterSelfCheck` - 验证注册 "self" 健康检查
3. `AddDefaultHealthChecks_SelfCheck_ShouldReturnHealthy` - 验证 "self" 检查返回健康状态
4. `ConfigureOpenTelemetry_ShouldRegisterTracingAndMetrics` - 验证配置 OpenTelemetry 追踪和指标
5. `MapDefaultEndpoints_InDevelopment_ShouldMapHealthCheckEndpoints` - 验证开发环境映射健康检查端点
6. `MapDefaultEndpoints_InProduction_ShouldNotMapHealthCheckEndpoints` - 验证生产环境不映射健康检查端点
7. `AddServiceDefaults_ShouldConfigureHttpClientWithResilience` - 验证配置 HttpClient 弹性
8. `AddServiceDefaults_ShouldBeIdempotent` - 验证 AddServiceDefaults 幂等性
9. `AddWolverineDefaults_ShouldRegisterWolverineServices` - 验证注册 Wolverine 服务
10. `AddWolverineDefaults_ShouldEnableFluentValidation` - 验证启用 FluentValidation
11. `AddWolverineDefaults_WithServiceDefaults_ShouldWorkTogether` - 验证 ServiceDefaults 和 WolverineDefaults 协同工作
12. `AddWolverineDefaults_CalledMultipleTimes_ShouldThrow` - 验证多次调用 AddWolverineDefaults 抛出异常

**共计: 19 个测试用例**

## 如何运行测试

### 🎯 推荐：使用便捷脚本（新增）

**快捷方式**: 我们提供了便捷的 Bash 脚本来运行不同层级的测试。

**Quick access**: We provide convenient Bash scripts to run different test layers.

```bash
cd src/Wolverine/scripts

# 快速烟雾测试（无需 Docker，< 2 秒）
./test-smoke.sh

# 集成测试（需要 Docker，约 10 秒）
./test-integration.sh

# 所有测试（烟雾 + 集成，约 30 秒）
./test-all.sh

# 模拟 CI 环境（包含构建、格式检查）
./test-ci.sh
```

📖 **详细文档**: [scripts/README.md](scripts/README.md)

---

### 推荐：分层测试策略

#### 🚀 快速验证（无需 Docker，< 10 秒）
运行第一层测试（ServiceDefaults + Bootstrapper 烟雾测试）：

```bash
cd src/Wolverine

# 使用脚本（推荐）
./scripts/test-smoke.sh

# 或手动运行
# ServiceDefaults 测试
dotnet test Aspire/Zss.BilliardHall.Wolverine.ServiceDefaults.Tests

# Bootstrapper 烟雾测试
dotnet test Bootstrapper.Tests --filter "Category=Unit"
```

#### 🐳 完整验证（需要 Docker，约 30 秒）
运行第一层 + 第二层测试（包含 Testcontainers 集成测试）：

```bash
cd src/Wolverine

# 使用脚本（推荐）
./scripts/test-all.sh

# 或手动运行
# 确保 Docker 正在运行
docker info

# 运行所有非 AppHost 测试
dotnet test --filter "Category!=RequiresDocker|Category=Unit"

# 或分别运行
dotnet test Aspire/Zss.BilliardHall.Wolverine.ServiceDefaults.Tests
dotnet test Bootstrapper.Tests
```

#### 🏗️ 端到端验证（需要 Docker + Aspire DCP，较慢）
运行第三层 AppHost 测试（**注意**：需要手动移除 `Skip` 属性）：

```bash
cd src/Wolverine

# 1. 编辑 AppHostIntegrationTests.cs，移除 Skip 属性
# 2. 运行测试
dotnet test Aspire/Zss.BilliardHall.Wolverine.AppHost.Tests
```

### 传统方式

#### 1. 运行所有测试（包括 AppHost，如果启用）

```bash
cd src/Wolverine
dotnet test
```

#### 2. 列出所有测试（不运行）

```bash
cd src/Wolverine
dotnet test --list-tests
```

#### 3. 运行特定测试项目

```bash
cd src/Wolverine

# ServiceDefaults 测试
dotnet test Aspire/Zss.BilliardHall.Wolverine.ServiceDefaults.Tests/Zss.BilliardHall.Wolverine.ServiceDefaults.Tests.csproj

# Bootstrapper 测试
dotnet test Bootstrapper.Tests/Zss.BilliardHall.Wolverine.Bootstrapper.Tests.csproj

# AppHost 测试
dotnet test Aspire/Zss.BilliardHall.Wolverine.AppHost.Tests/Zss.BilliardHall.Wolverine.AppHost.Tests.csproj
```

#### 4. 按类别过滤

```bash
cd src/Wolverine

# 只运行单元测试
dotnet test --filter "Category=Unit"

# 只运行集成测试
dotnet test --filter "Category=Integration"

# 排除需要 Docker 的测试
dotnet test --filter "Category!=RequiresDocker"
```

#### 5. 运行特定测试类

```bash
cd src/Wolverine
dotnet test --filter "FullyQualifiedName~MartenExtensionsTests"
dotnet test --filter "FullyQualifiedName~BootstrapperSmokeTests"
```

#### 6. 运行特定测试方法

```bash
cd src/Wolverine
dotnet test --filter "FullyQualifiedName=Zss.BilliardHall.Wolverine.Bootstrapper.Tests.BootstrapperSmokeTests.BuildApp_WithValidArgs_ShouldSucceed"
```

#### 7. 运行测试并显示详细输出

```bash
cd src/Wolverine
dotnet test --verbosity normal
```

或者更详细：

```bash
cd src/Wolverine
dotnet test --verbosity detailed
```

### CI/CD 测试策略

#### GitHub Actions CI
```yaml
# PR 检查：快速反馈（无 Docker）
- ServiceDefaults.Tests（19个测试）
- Bootstrapper.Tests --filter "Category=Unit"（6个测试）
总耗时：< 10 秒

# 手动触发或 push main：完整验证
- 上述所有测试
- AppHost.Tests（可选，需手动启用）
总耗时：视 AppHost 启动速度而定
```

## 测试项目依赖

测试项目依赖以下 NuGet 包：

- `Microsoft.NET.Test.Sdk` - .NET 测试 SDK
- `xunit` - xUnit 测试框架
- `xunit.runner.visualstudio` - Visual Studio 测试运行器
- `FluentAssertions` - 流畅断言库
- `Microsoft.AspNetCore.Mvc.Testing` - ASP.NET Core 测试支持
- `Marten` - Marten 文档数据库
- `WolverineFx.Http` - Wolverine HTTP 支持

## 项目引用

测试项目引用：

- `Zss.BilliardHall.Wolverine.ServiceDefaults` - 被测试的 ServiceDefaults 项目

## 未来测试计划

根据 `README.md` 中的待办事项，未来将添加以下测试：

### 阶段一：Bootstrapper 配置测试
- Wolverine 配置测试
- Marten 文档数据库配置测试
- Serilog 日志配置测试
- HTTP 端点扫描测试

### 阶段二：Members 模块测试
- RegisterMember 功能测试
- GetMember 查询测试
- MemberRegistered 事件测试

### 阶段三：其他模块测试
- Tables 模块测试
- Sessions 模块测试

### 阶段四：集成测试
- 创建集成测试项目
- 核心功能端到端测试
- 应用启动验证测试

## 测试最佳实践

参考以下开发规范：

- [代码风格规范](../../../docs/06_开发规范/代码风格.md)
- [FluentValidation集成指南](../../../docs/06_开发规范/FluentValidation集成指南.md)
- [单元测试计划](../../../docs/09_测试方案/单元测试计划.md)
- [集成测试计划](../../../docs/09_测试方案/集成测试计划.md)

### 测试命名规范

测试方法命名遵循模式：`MethodName_Condition_ExpectedResult`

示例：
- `AddMartenDefaults_WithValidConnectionString_ShouldRegisterMartenServices`
- `AddServiceDefaults_ShouldRegisterAllRequiredServices`

### 测试组织

- 使用 **Arrange-Act-Assert** 模式组织测试
- 每个测试聚焦单一功能点
- 使用 FluentAssertions 编写清晰的断言
- 添加中英文注释说明测试意图

## 测试统计总览

| 测试项目 | 测试数量 | 执行时间 | Docker 需求 | CI 运行 |
|---------|---------|---------|------------|---------|
| ServiceDefaults.Tests | 19 | < 5 秒 | ❌ 无需 | ✅ 每次 PR |
| Bootstrapper.Tests (Unit) | 6 | < 2 秒 | ❌ 无需 | ✅ 每次 PR |
| Bootstrapper.Tests (Integration) | 5 | 5-10 秒 | ✅ 需要 | ⏸️ 本地手动 |
| AppHost.Tests | 3 | 30+ 秒 | ✅ 需要 + DCP | ❌ 手动触发 |
| **总计** | **33** | **变化** | - | - |

## 测试最佳实践

参考以下开发规范：

- [代码风格规范](../../docs/06_开发规范/代码风格.md)
- [FluentValidation集成指南](../../docs/06_开发规范/FluentValidation集成指南.md)
- [单元测试计划](../../docs/09_测试方案/单元测试计划.md)
- [集成测试计划](../../docs/09_测试方案/集成测试计划.md)

### 测试命名规范

测试方法命名遵循模式：`MethodName_Condition_ExpectedResult`

示例：
- `AddMartenDefaults_WithValidConnectionString_ShouldRegisterMartenServices`
- `BuildApp_WithValidArgs_ShouldSucceed`
- `Marten_CanPersistAndRetrieveDocument`

### 测试组织

- 使用 **Arrange-Act-Assert** 模式组织测试
- 每个测试聚焦单一功能点
- 使用 FluentAssertions 编写清晰的断言
- 添加中英文注释说明测试意图
- 使用 `[Trait]` 标记测试类别

## 架构变更记录

### 2026-01-11: 引入分层测试架构 ✨

**问题**：AppHost 集成测试依赖 Docker + Aspire DCP，在 CI 环境中超时/不稳定。

**解决方案**：
1. **抽取 Host 构建逻辑** → `BootstrapperHost` 静态类
2. **创建轻量测试项目** → `Bootstrapper.Tests` + Testcontainers
3. **分层测试策略**：
   - 第一层：单元测试（无外部依赖）
   - 第二层：轻量集成测试（仅需 Docker）
   - 第三层：端到端测试（完整 AppHost）
4. **CI 优化**：PR 只运行快速测试，AppHost 测试改为可选

**收益**：
- ✅ CI 测试时间从 30+ 秒降至 < 10 秒
- ✅ 测试覆盖率提升（新增 11 个 Bootstrapper 测试）
- ✅ 降低 CI 失败率（移除对 Aspire DCP 的强依赖）
- ✅ 本地开发体验改善（快速反馈）

## 持续集成

测试在 CI/CD 流程中自动运行。**必须所有第一层测试通过才能合并 PR**。

GitHub Actions Workflow: [.github/workflows/ci.yml](../../.github/workflows/ci.yml)

## 相关文档

- [Wolverine快速上手指南](../../docs/03_系统架构设计/Wolverine快速上手指南.md)
- [Wolverine模块化架构蓝图](../../docs/03_系统架构设计/Wolverine模块化架构蓝图.md)
- [ServiceDefaults集成指南](../../docs/06_开发规范/ServiceDefaults集成指南.md)
- [测试方案文档](../../docs/09_测试方案/)
- [Bootstrapper.Tests README](Bootstrapper.Tests/README.md)
- [AppHost.Tests README](Aspire/Zss.BilliardHall.Wolverine.AppHost.Tests/README.md)

---

**最后更新**: 2026-01-11  
**版本**: 2.0.0 - 分层测试架构  
**测试项目**: 3 个（ServiceDefaults.Tests, Bootstrapper.Tests, AppHost.Tests）  
**总测试数**: 33 个
