...
namespace Zss.BilliardHall.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("BilliardHall") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor
            {
                Name = "BilliardHall", DisplayName = "BilliardHall API", Resources = { "BilliardHall" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["BilliardHall_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["BilliardHall_App:RootUrl"]
                ?.TrimEnd('/');
            await CreateApplicationAsync(OpenIddictConstants.ApplicationTypes.Web,
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["BilliardHall_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["BilliardHall_Swagger:RootUrl"]
                ?.TrimEnd('/');
        ...
        string? logoUri = null)
    {
        ...
        {
            // EndSession endpoint permission - commented out for compatibility
        ...
        foreach (var grantType in grantTypes)
        {
            ...
            if (grantType == OpenIddictConstants.GrantTypes.DeviceCode)
            {
                ...
                // DeviceAuthorization endpoint permission - commented out for compatibility
        ...
        {
            client.RedirectUris = JsonSerializer.Serialize(application.RedirectUris.Select(q => q
                .ToString()
                .RemovePostFix("/")));
            client.PostLogoutRedirectUris = JsonSerializer.Serialize(application.PostLogoutRedirectUris.Select(q => q
                .ToString()
                .RemovePostFix("/")));
    ...
    private bool HasSameRedirectUris(OpenIddictApplication existingClient, AbpApplicationDescriptor application)
    {
        return existingClient.RedirectUris ==
               JsonSerializer.Serialize(application.RedirectUris.Select(q => q
                   .ToString()
                   .RemovePostFix("/")));
    }
    ...
    private bool HasSameScopes(OpenIddictApplication existingClient, AbpApplicationDescriptor application)
    {
        return existingClient.Permissions ==
               JsonSerializer.Serialize(application.Permissions.Select(q => q
                   .ToString()
                   .TrimEnd('/')));
    }
}
