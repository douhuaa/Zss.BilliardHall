---
title: "数据迁移方案"
description: "EF Core Code First 数据迁移完整指南和最佳实践"
section: "5.5"
version: "1.0.0"
author: "开发团队"
maintainer: "开发工程师"
...
category: "数据库设计"
level: "必读"
audience: ["开发工程师", "运维工程师"]
keywords: ["EF Core Migrations", "数据迁移", "Code First", "数据库版本管理", "迁移脚本", "数据种子"]
tags: ["efcore", "migrations", "database", "version-control"]
status: "完成"
dependencies: ["05_数据库设计/表结构定义.md"]
related_docs: ["05_数据库设计/关键表说明.md", "05_数据库设计/EFCore开发工作流程.md"]
reading_time: "15分钟"
difficulty: "中级"
---
...
**导航路径**: [🏠 项目文档首页](../自助台球系统项目文档.md) > [📊 数据库设计](README.md) > 🔄 数据迁移方案

<!-- Keywords for Search -->
**关键词**: `EF Core Migrations` `数据迁移` `Code First` `数据库版本管理` `迁移脚本` `数据种子`

## 概述

本项目采用 EF Core Code First 方式进行数据库设计和版本管理，通过 Entity Framework Core Migrations 实现数据库结构的版本化管理。

> 💡 **相关章节**：详细的实体定义请参考 [5.2 表结构定义](表结构定义.md)，业务逻辑说明请参考 [5.3 关键表说明](关键表说明.md)。

## EF Core Code First 开发流程
...
- 迁移名称使用英文驼峰命名：`AddUserModule`、`UpdatePaymentSchema`
- 提交信息使用中文：`feat: 添加用户模块数据库迁移`
...
protected override void Up(MigrationBuilder migrationBuilder)
{
    ...
    // 更新现有数据
    migrationBuilder.Sql("UPDATE Users SET NewColumn = '默认值' WHERE NewColumn IS NULL");
...
2. **数据丢失风险**：删除列或表的迁移
   - 解决方案：先备份数据，使用 `Down` 方法提供回滚逻辑

3. **性能问题**：大表的结构变更
   - 解决方案：分批执行，使用原生 SQL 语句
...
### 相关章节
- [6.4 Git 分支规范](../06_开发规范/Git分支规范.md)
- [6.5 Code Review 流程](../06_开发规范/CodeReview流程.md)
