...
| 💳 [支付模块](支付模块.md) | 支付流程、退款管理 | 幂等性、事务处理 | 支付集成必读 | 🔄 legacy（待重写）|
| 🖥️ [设备管理模块](设备管理模块.md) | 设备控制、状态监控 | 物联网协议、实时通信 | 硬件对接必读 | 🔄 legacy（待重写）|
| 📊 [报表与统计模块](报表与统计模块.md) | 数据分析、报表生成 | 数据聚合、可视化 | 数据分析参考 | 🔄 legacy（待重写）|

**版本说明**:
- ✅ **v3.0.0**: 完全基于 Wolverine 架构重写，无旧架构内容（如 Members 模块）
- ✅ **v2.0.0**: 完全基于 Wolverine 架构重写，独立文档（如 Sessions、Billing 模块）
- 🔄 **legacy**: 已移至 [legacy/](legacy/) 目录，待后续重写
- 📚 **v1.0.0**: 旧版本文档，已归档至 [legacy/](legacy/) 目录
