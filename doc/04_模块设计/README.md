...
> ⚠️ **架构迁移说明**
> 
> 本项目正在从 ABP 分层架构迁移到 **Wolverine 垂直切片架构**。本目录中的个别模块文档（如会员管理模块.md、台球桌计费模块.md 等）仍使用旧的分层架构术语，**正在逐步更新中**。
> 
> **推荐阅读顺序**:
...
|------|----------|----------|----------|---------|
| 🏃 [会员管理模块](会员管理模块.md) | 会员体系、积分管理 | 等级算法、权限控制 | 业务开发必读 | 🔄 待更新 |
| 💰 [台球桌计费模块](台球桌计费模块.md) | 计费规则、套餐管理 | 计时算法、价格策略 | 核心业务模块 | 🔄 待更新 |
| 💳 [支付模块](支付模块.md) | 支付流程、退款管理 | 幂等性、事务处理 | 支付集成必读 | 🔄 待更新 |
| 🖥️ [设备管理模块](设备管理模块.md) | 设备控制、状态监控 | 物联网协议、实时通信 | 硬件对接必读 | 🔄 待更新 |
| 📊 [报表与统计模块](报表与统计模块.md) | 数据分析、报表生成 | 数据聚合、可视化 | 数据分析参考 | 🔄 待更新 |

> 💡 **提示**: 以上模块文档主要用于理解**业务需求**，具体**架构实现**请参考 [Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)
...
*最后更新: 2024-01-15 | 版本: v1.1.0 | 架构迁移状态: 🔄 个别模块文档待更新至 Wolverine 架构*