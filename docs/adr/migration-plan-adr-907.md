...
- [x] 修复 ADR_920 和 ADR_910 的形式化断言
- [x] 为 ADR-0905 和 ADR-0901 声明 Non-Enforceable（治理层元规则，不需要架构测试）
- [x] 更新所有测试的 DisplayName 格式（27 个文件，230+ 处更新）
- [x] 更新所有测试的失败消息格式
- [x] 修复 ADR-907 文档中的"仅文档约束"声明
- ⚠️ 部分测试文件存在 DisplayName 与失败消息 RuleId 不一致（需要深入重构，超出当前整改范围）

### 第三阶段：ADR 文档审查 ✅

- [x] 审查 ADR-0905 和 ADR-0901（已声明 Non-Enforceable）
- [x] 修复 ADR-907 文档声明
- [x] 确保所有 Final/Active ADR 要么有测试，要么声明 Non-Enforceable

### 整改结果

**测试通过率提升**：
- ADR-907 测试：从 61.9% 提升到 81.0%（+19.1%）
- 整体架构测试：95.8% 通过率（203/212 通过）

**剩余问题**：
- 4 个 ADR-907 失败是检测器技术限制（实际格式正确，但正则无法匹配错误累积模式）
- 部分测试文件 RuleId 编号不系统化，需要深入重构（超出当前整改范围）
...
本整改计划基于 2026-02-02 的 ADR-907 测试执行结果生成。

**最后更新**：2026-02-02
**整改状态**：主要任务已完成（P0 和 P1 完成，P2 已评估）
**完成人员**：GitHub Copilot（在 PR #235 分支上执行）
