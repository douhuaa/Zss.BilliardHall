...
| 模糊表达 | 可判定表达 |
|---------|-----------|
| ❌ "应该记录关键日志" | ✅ "所有 Handler 必须记录带 CorrelationId 的日志" |
| ❌ "尽量使用异步" | ✅ "跨模块通信必须使用异步事件" |
| ❌ "合理组织代码" | ✅ "测试类必须位于与源码镜像的目录结构" |
| ❌ "避免过度耦合" | ✅ "模块禁止直接引用其他模块的命名空间" |
...
2. **场景 2**：开发者修改 Handler 构造函数
   ...
   - 提示：ADR-XXX.2 禁止依赖 Singleton 有状态服务

...（4-5 个场景）

---

## 必须阻止的反模式（Anti-Patterns）
