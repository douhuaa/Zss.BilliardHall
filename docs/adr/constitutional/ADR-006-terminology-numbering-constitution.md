...
| 宪法层 ADR       | ADR-001~009，系统根基，不可推翻，只能细化，破例需特批                  | Constitutional ADR |
| 治理层 ADR       | ADR-900~999，架构过程、测试、CI、审批、破例管理 | Governance ADR     |
...
**规则**：
- 编号段直接对应架构层级：
  - `ADR-001~009`：宪法层，系统根基
  - `ADR-900~999`：治理层，流程管理
...
**判定**：
- ❌ 使用非3位格式（如 ADR-1, ADR-0001, ADR-10000）
...
- ADR-006.2: 文件名与编号一致性检查（3位格式）
- ADR-006.3: 所有ADR必须使用3位编号格式
- ADR-006.4: ADR 文件目录归属检查
- ADR-006.5: 术语使用一致性审查（人工）
...
- ❌ **禁止使用非标准编号格式**：如 `ADR_001`、`adr-1`、`ADR-A-001`、`ADR-0001`（4位），正确格式为 `ADR-001`（3位数字）
