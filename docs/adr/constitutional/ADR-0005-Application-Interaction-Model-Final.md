...
| 契约（Contract）  | 模块间只读、单向、版本化的数据 DTO，仅用于信息传递，不含业务决策  | Contract       |
| 领域实体          | 业务内聚的复杂类型，封装业务不变量，不允许跨模块共享            | Domain Entity  |
| 领域事件          | 描述已发生的业务事实，供其他模块异步订阅                   | Domain Event   |
| 模块间通信         | 只允许通过领域事件、契约 DTO、原始类型（Guid/string/int） | Inter-Module Communication |
