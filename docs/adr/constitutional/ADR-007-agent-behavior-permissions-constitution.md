...
### Agent 禁止的语义行为（ADR-007.2~0007.6）
...
4. **禁止 Prompts 作为裁决依据**（ADR-007.5）
   - 裁决必须引用 ADR 正文
   - Prompts 仅作参考资料链接

5. **禁止发明架构规则**（ADR-007.6）
...
所有规则通过 `src/tests/ArchitectureTests/ADR/ADR_007_Architecture_Tests.cs` 强制验证：
...
- [ADR-900：ADR 新增与修订流程](../governance/ADR-900-architecture-tests.md) - Agent 变更流程参考 ADR 分级权限
