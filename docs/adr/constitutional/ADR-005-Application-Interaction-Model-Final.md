...
> 🔒 **统一铁律**：
> 
> ADR-005 中，所有可执法条款必须具备稳定 RuleId，格式为：
> ```
...
**判定**：
- ❌ Endpoint 包含 if/switch 等业务逻辑
- ✅ Endpoint 只做参数映射和 Handler 调用
...
**判定**：
- ❌ 同一 Handler 同时处理 Command 和 Query
- ✅ Command 和 Query 独立 Handler
