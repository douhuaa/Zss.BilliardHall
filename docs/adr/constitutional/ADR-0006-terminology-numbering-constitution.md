...
> **权威性边界**：本术语表仅对"架构裁决语义"负责，不承担技术百科职责。非裁决性术语可存在于其他 ADR，不强制收录于本表。
...
| 治理层 ADR       | ADR-0000, 900~999，架构过程、测试、CI、审批、破例管理。允许包含流程、工具、人员职责的混合规则，只要围绕治理目标且不越权定义其他层级的技术实现细节 | Governance ADR     |
...
- **范围约束**：ADR-0000 不得新增具体治理规则、技术约束或流程细节，仅允许定义"治理元机制"和"测试执行模型"

#### ADR-0006：本 ADR

- **编号**：`ADR-0006`（宪法层最后一个已使用编号）
- **归属**：宪法层
- **地位**：术语与编号的元规则
- **原因**：编号体系本身属于宪法层
...
**宪法层 ADR 编号区间为 ADR-0001~0009，其中：**

- **ADR-0001~0006**：已使用
- **ADR-0007~0009**：为预留编号，非经架构委员会批准不得启用
...
> **责任边界**：本 ADR 仅定义违规判定依据，具体执行流程由治理层 ADR 负责。

### 新增 ADR 违规

当新增 ADR 不符合编号规则时：

- **判定依据**：编号违反 ADR-0006 定义的规则
- **违规类型**：说明违反了哪条规则（引用 ADR-0006.X）
- **纠正建议**：提供正确的编号段和理由
...
| ADR-0006.6 | 术语使用一致性             | 人工语义审查（工具可辅助）    | L3  |
