...
### 编号语义约束（ADR-0006.3, 0006.4, 0006.5）

**规则**：
- 治理层 ADR 必须使用 0000 或 900~999，禁止使用 100~399
- 技术层 ADR 必须使用 300~399，禁止使用 900~999
- 细化层（结构/运行/技术）不得引入宪法级约束
- ADR 编号一经分配不可更改（除非违规严重需特批）

**判定**：
- ❌ 层级跨界（治理用 300 段、技术用 900 段）
- ❌ 细化层引入跨模块强制约束
- ❌ 擅自更改已分配编号
- ✅ 编号语义边界清晰
...
|------------|---------------------|------------------|----------------------------------------------|------|
| ADR-0006.1 | 编号段必须匹配架构层级         | L1 - 脚本检查        | ADR_Numbering_Should_Match_Layer             | ✅    |
| ADR-0006.2 | ADR 文件名必须与编号一致      | L1 - 脚本检查        | ADR_Filename_Should_Match_Number             | ✅    |
| ADR-0006.3 | 治理层禁止使用 100~399     | L1 - 脚本检查        | Governance_ADRs_Should_Not_Use_100_399       | ✅    |
| ADR-0006.4 | 技术层禁止使用 900~999     | L1 - 脚本检查        | Technical_ADRs_Should_Not_Use_900_999        | ✅    |
| ADR-0006.5 | ADR 编号不重复           | L1 - 脚本检查        | ADR_Numbers_Should_Be_Unique                 | ✅    |
