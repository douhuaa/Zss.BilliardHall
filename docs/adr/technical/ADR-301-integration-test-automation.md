...
### ADR-301.1：容器化依赖管理（Rule）

#### ADR-301.1.1 外部依赖容器化管理【必须架构测试覆盖】
...
#### ADR-301.1.2 容器启动超时与健康检查【必须架构测试覆盖】
...
#### ADR-301.1.3 本地与 CI 环境差异化策略【必须架构测试覆盖】
...
### ADR-301.2：测试隔离与并行执行（Rule）

#### ADR-301.2.1 测试类隔离规则【必须架构测试覆盖】
...
|---------|--------|---------|--------------|
| **ADR-301.1.1** | L2 | 检查集成测试项目引用 TestContainers 包 | §ADR-301.1.1 |
| **ADR-301.1.2** | L1 | ArchitectureTests 验证容器健康检查配置 | §ADR-301.1.2 |
| **ADR-301.1.3** | L2 | CI 中验证环境差异化策略 | §ADR-301.1.3 |
| **ADR-301.2.1** | L1 | ArchitectureTests 验证测试类隔离 | §ADR-301.2.1 |
| **ADR-301.2.2** | L3 | 人工审查测试清理完整性 | §ADR-301.2.2 |
| **ADR-301.2.3** | L1 | CI 运行并行测试验证 | §ADR-301.2.3 |
