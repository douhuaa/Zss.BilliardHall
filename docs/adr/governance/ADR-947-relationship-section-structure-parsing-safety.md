...
**状态**：✅ Accepted  
**版本**：1.0  
**级别**：治理级补充约束（Governance Supplement）  
**适用范围**：所有 ADR 文档  
**目的**：彻底消除解析歧义、自指、模板污染、循环链放大  
**生效时间**：即刻
...
| 规则编号 | 执行级别 | 测试/手段 | 说明 |
|---------|---------|----------|------|
| 条款 1 | **L1** | CI 脚本检测 | 检测多个 `## 关系声明` |
| 条款 2 | **L1** | CI 脚本检测 | 验证关系区边界清晰 |
| 条款 3 | **L1** | CI 脚本检测 | 检测非关系区的 ADR 编号 |
| 条款 4 | **L1** | CI 脚本检测 | 检测同编号多文件 |
| 条款 5 | **L1** | `detect-circular-dependencies.sh` | 检测循环依赖 |

**执行工具**：
- 架构脚本**只**解析：`## 关系声明` 下的 bullet list
- 任意违规：
  - CI ❌
  - 架构测试 ❌
  - PR 不可合并
...
**拒绝的方案**：
- ❌ **不靠"写清楚点"解决问题** —— 人会犯错，脚本不会
- ❌ **不靠更复杂的正则** —— 结构错误不该靠 regex 修补
...
> **ADR 如果不能被机器 100% 无歧义解析，那它就不是架构宪法，只是 README 的高级版本。**
