...
2. 测试目录必须按 ADR 编号分组
   - `/ADR-XXXX/`
3. **规则独立测试（避免类膨胀）**
   - 每一条具体架构规则必须使用独立的测试类进行验证
   - 禁止多个规则混合在同一个测试类
   - 单个测试类或文件仅允许覆盖一个 ADR 的一个规则
4. 禁止跨 ADR 混合测试
   - 单个测试类或文件仅允许覆盖一个 ADR
5. **测试类命名必须显式绑定 ADR 和规则编号**
   - 格式：`ADR_<编号>_<规则编号>_Tests`
   - 示例：`ADR_0907_1_Tests`、`ADR_0907_2_Tests`
   - 兼容格式（单一规则或汇总测试）：`ADR_<编号>_Architecture_Tests`
6. **测试方法命名必须映射规则下的具体行为场景**
   - 每个测试方法应对应规则下细化的行为场景
   - 命名需能映射具体规则及行为
   - 推荐格式：`<场景描述>` 或 `Rule_<场景编号>_<行为描述>`
   - 在 DisplayName 中明确标注 ADR 和规则：`ADR-<编号>.<规则编号>: <描述>`
7. 测试失败信息必须包含 ADR 编号与子规则
8. ArchitectureTests 不得为空、占位或弱断言
9. 不得 Skip、条件禁用测试（除非走破例机制）
...
|ADR-907.3|L1|规则独立测试校验（避免类膨胀）|
|ADR-907.4|L1|测试类与 ADR 映射校验|
|ADR-907.5|L1|测试类命名正则校验（支持规则编号）|
|ADR-907.6|L1|测试方法映射行为场景校验|
|ADR-907.7|L1|失败信息 ADR 溯源校验|
|ADR-907.8|L2|最小断言数量与语义检测|
|ADR-907.9|L1|Skip / 条件禁用检测|
|ADR-907.10|L1|CI / Analyzer 自动注册校验|
|ADR-907.11|L1|L1 阻断 / L2 告警策略执行|
|ADR-907.12|L1|破例与偿还机制记录|
|ADR-907.13|L2|ADR 生命周期同步校验|
