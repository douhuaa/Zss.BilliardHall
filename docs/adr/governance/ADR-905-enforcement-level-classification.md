...
**版本**：2.0  
**级别**：治理层 / 执行分级标准  
**适用范围**：所有架构规则（特别是 ADR-0005）的验证和执行  
**生效时间**：即刻
...
| 规则编号 | 执行级 | 执法方式 |
|---------|--------|---------|
| ADR-905.1 | L2 | ADR 编写时的执行级别声明检查（Code Review） |
| ADR-905.2 | L1 | NetArchTest 架构测试（CI 阻断） |
| ADR-905.3 | L2 | Roslyn Analyzer（警告）+ 人工审查 |
| ADR-905.4 | L3 | PR Template + 架构师审查 + ARCH-VIOLATIONS 记录 |
| ADR-905.5 | L2 | Code Review + 教育培训 |

### 执行时机

- **CI 阶段**：L1 规则自动阻断
- **本地开发**：L2 Analyzer 提供实时警告
- **PR Review**：L3 规则人工审查
- **季度审计**：破例记录和执行效果评估
