...
**依赖（Depends On）**：
- [ADR-001：模块化单体与垂直切片架构](../constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md) - 本 ADR 基于模块隔离规则
...
**相关（Related）**：
- [ADR-005：应用内交互模型](../constitutional/ADR-005-Application-Interaction-Model-Final.md) - 相关的通信模式
```
...
| 关系类型 | 使用场景 | 示例 |
|---------|---------|------|
| **依赖（Depends On）** | 你的 ADR 基于另一 ADR 的规则或概念 | 你的 ADR 定义 Handler 生命周期，需要基于 ADR-005 的 CQRS 规则 |
| **相关（Related）** | 两个 ADR 相关但不存在依赖 | 你的 ADR 定义日志格式，ADR-970 定义日志集成，两者相关但不依赖 |
...
```markdown
❓ 问题：ADR-201 定义 Handler 生命周期，是否依赖 ADR-005？

✅ 答案：是，应该依赖
理由：Handler 生命周期基于 ADR-005 的 CQRS 规则，必须遵循 Command/Query 的定义

❓ 问题：ADR-340 定义领域事件，是否依赖 ADR-240 的数据库事务？
...
你需要：
1. 在 ADR-201 中添加：依赖 ADR-005
2. 在 ADR-005 中添加：被依赖 ADR-201
```
...
**依赖（Depends On）**：
- [ADR-005：应用内交互模型](../constitutional/ADR-005-Application-Interaction-Model-Final.md) - 本 ADR 基于 CQRS 规则定义 Handler 生命周期
```
...
**被依赖（Depended By）**：
- [ADR-201：Handler 生命周期管理](../runtime/ADR-201-handler-lifecycle.md)
```
...
```bash
# 检查 ADR-005-v1 的状态
grep "状态" docs/adr/constitutional/ADR-005-v1.md
```
...
根据 ADR-940.2：
- ADR-005-v1 已被 ADR-005-v2 替代（状态：Superseded）
...
✅ 正确：
**依赖（Depends On）**：
- [ADR-005-v2：应用内交互模型（最新版）](../constitutional/ADR-005-v2.md)
```
...
# 输出示例：
❌ 发现循环依赖：
ADR-201 → ADR-005 → ADR-340 → ADR-201
```
...
```markdown
❌ 循环依赖：
ADR-201（Handler 生命周期）依赖 ADR-340（领域事件）
ADR-340（领域事件）依赖 ADR-201（Handler 发布事件）

✅ 解决方案：
创建 ADR-199（事件基础架构）包含公共规则
ADR-201 和 ADR-340 都依赖 ADR-199
```
...
```markdown
在 ADR-005 中找到"关系声明（Relationships）"章节：

## 关系声明（Relationships）

**被依赖（Depended By）**：
- [ADR-201：Handler 生命周期](../runtime/ADR-201-handler-lifecycle.md)
```
...
**被依赖（Depended By）**：
- [ADR-201：Handler 生命周期](../runtime/ADR-201-handler-lifecycle.md)
- [ADR-340：领域事件模式](../runtime/ADR-340-domain-events.md)  ← 新增
```

3. **确保双向一致**：

```
提醒：你还需要在 ADR-340 中添加：
...
```markdown
**依赖（Depends On）**：
- [ADR-905-enforcement-Levels](链接)  ← 已被废弃
```
