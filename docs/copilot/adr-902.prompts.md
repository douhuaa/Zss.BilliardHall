...
### Front Matter（必需）
- ✅ 所有 ADR 必须包含 YAML Front Matter
- ✅ 必需字段：adr, title, status, level, deciders, date, version, maintainer, reviewer, supersedes, superseded_by
- ❌ status 只能是：Draft, Accepted, Final, Superseded
- ❌ level 只能是：Constitutional, Governance, Structure, Runtime, Technical
...
### 规则条目格式
- ✅ 每条规则必须是独立的三级标题
- ✅ 格式：### ADR-XXX.Y:L? <规则标题>（中文别名可选）
- ✅ 规则编号必须连续：ADR-XXX.1, ADR-XXX.2, ...
- ❌ 不允许合并多条规则在一个章节
...
### L2 指导原则
- ✅ 约束性规则只出现在 Decision 章节
- ⚠️ 其他章节避免使用：必须、MUST、禁止、不允许、不得、SHALL NOT
- ✅ 在 Focus、Glossary 等章节使用描述性语言
