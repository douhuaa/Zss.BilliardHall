# ADR-002：Platform / Application / Host 三层启动体系 - Copilot 提示词库

**关联 ADR**：[ADR-002](../adr/constitutional/ADR-002-platform-application-host-bootstrap.md)  
**测试类**：`ADR_002_Architecture_Tests.cs`  
**版本**：1.0  
**最后更新**：2026-01-21

---

## 一、当我在写什么时，你应该提醒我哪些 ADR-002 约束？
...
```
根据 ADR-002，依赖方向必须是：Host → Application → Platform
...
```
根据 ADR-002，Host 层不应该包含业务逻辑或业务类型。
...
```
根据 ADR-002，Program.cs 不应超过 30 行。
...
| ADR 约束      | 测试方法                                                      | 测试内容                                         |
|-------------|-----------------------------------------------------------|----------------------------------------------|
| ADR-002.1  | `Platform_Should_Not_Depend_On_Application`               | Platform 不应依赖 Application                    |
| ADR-002.2  | `Platform_Should_Not_Depend_On_Host`                      | Platform 不应依赖 Host                           |
| ADR-002.3  | `Platform_Should_Not_Depend_On_Modules`                   | Platform 不应依赖任何 Modules                      |
| ADR-002.4  | `Platform_Should_Have_Single_Bootstrapper_Entry_Point`    | Platform 应有唯一的 PlatformBootstrapper 入口       |
| ADR-002.5  | `Application_Should_Not_Depend_On_Host`                   | Application 不应依赖 Host                        |
| ADR-002.6  | `Application_Should_Not_Depend_On_Modules`                | Application 不应依赖任何 Modules                   |
| ADR-002.7  | `Application_Should_Have_Single_Bootstrapper_Entry_Point` | Application 应有唯一的 ApplicationBootstrapper 入口 |
| ADR-002.8  | `Application_Should_Not_Use_HttpContext`                  | Application 不应包含 HttpContext 等 Host 专属类型     |
| ADR-002.9  | `Host_Should_Not_Depend_On_Modules`                       | Host 不应依赖任何 Modules                          |
| ADR-002.10 | `Host_Should_Not_Contain_Business_Types`                  | Host 不应包含业务类型                                |
| ADR-002.11 | `Host_Csproj_Should_Not_Reference_Modules`                | Host 项目文件不应引用 Modules                        |
| ADR-002.12 | `Program_Cs_Should_Be_Concise`                            | Program.cs 应保持简洁（≤ 50 行）                     |
| ADR-002.13 | `Program_Cs_Should_Only_Call_Bootstrapper`                | Program.cs 只应调用 Bootstrapper                 |
| ADR-002.14 | `Verify_Complete_Three_Layer_Dependency_Direction`        | 验证完整的三层依赖方向                                  |
...
```csharp
[Fact(DisplayName = "ADR-002.1: Platform 不应依赖 Application")]
public void Platform_Should_Not_Depend_On_Application()
{
    ...
    Assert.True(result.IsSuccessful,
        $"❌ ADR-002.1 违规: Platform 层不应依赖 Application 层\n\n" +
...
```csharp
[Theory(DisplayName = "ADR-002.11: Host 项目文件不应引用 Modules")]
...
public void Host_Csproj_Should_Not_Reference_Modules(string csprojPath)
{
    ...
    foreach (XmlNode reference in references)
    {
        ...
        {
            Assert.Fail(
                $"❌ ADR-002.11 违规: Host 项目不应直接引用 Modules\n\n" +
...
# 运行特定约束测试
dotnet test --filter "DisplayName~ADR-002.1"
dotnet test --filter "DisplayName~ADR-002.5"

# 查看详细输出
dotnet test --filter "FullyQualifiedName~ADR_002" --logger "console;verbosity=detailed"
```
...
```
1. 查看失败消息中的 ADR 编号
2. 参考 docs/copilot/adr-002.prompts.md 对应场景
3. 按照修复建议逐步修复
4. 重新运行测试验证
5. 如有疑问，参考 ADR-002 完整文档
```
...
- [ADR-002 完整文档](../adr/constitutional/ADR-002-platform-application-host-bootstrap.md)
- [架构测试类](../../src/tests/ArchitectureTests/ADR/ADR_002_Architecture_Tests.cs)
