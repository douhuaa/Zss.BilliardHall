# ADR-008 Copilot Prompts：文档编写与维护

> ⚖️ **权威声明**：本文件仅为辅助理解 ADR-008 的场景化示例，**不具备裁决权**。
>
> **所有架构判定必须基于 [ADR-008 正文](../adr/constitutional/ADR-008-documentation-governance-constitution.md)。**

**版本**：1.0  
**对应 ADR**：ADR-008（文档编写与维护宪法）  
**最后更新**：2026-01-25
...
- 架构裁决（必须参考相关 ADR 正文）
- 最终判定文档是否合规（以 ADR-008 正文为准）
- 文档分级的权威定义（参考 ADR-008 决策 1）

---

## 二、核心概念速查

### 文档分级与裁决力（ADR-008 决策 1）
...
本模块遵循以下 ADR：
- [ADR-001](../../adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md)：模块隔离
- [ADR-005](../../adr/constitutional/ADR-005-Application-Interaction-Model-Final.md)：Handler 模式

关于模块通信的具体规则，请参考 ADR-001 第 3.2 节。
```
...
ADR-001 规定模块间禁止直接引用，但在以下情况下可以例外：
...
**破例流程**：如需破例，必须按 ADR-900 的破例流程申请，记录在 ARCH-VIOLATIONS.md。

详见：[ADR-001 正文](../docs/adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md)
```
...
**文件**：`docs/adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md`
...
```markdown
> ⚠️ **无裁决力声明**：本文档仅供参考，不具备架构裁决权。
> 所有架构决策以 [ADR-001](../adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md) 为准。
...
发现冲突：
- ADR-001 正文：禁止直接引用（无例外）
- adr-001.prompts.md：允许测试代码例外

裁决：以 ADR-001 正文为准

处理：修正 adr-001.prompts.md，删除错误建议
```

**修正后的 Prompts**：

```markdown
> ⚖️ **权威声明**：本文件仅为辅助理解，若与 ADR-001 正文冲突，以正文为准。

## 测试中的模块引用

根据 ADR-001，模块间禁止直接引用，**测试代码也不例外**。

测试中访问其他模块数据，应该通过：
- 契约查询（只读）
- 测试专用的 Test Fixture（不包含业务逻辑）

详见：[ADR-001 第 3.2 节](../adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md#32-模块通信约束)
```
...
✅ 修正：根据 ADR-001，模块使用事件通信。详见：[ADR-001](link)
```
...
✅ 修正：如需破例，必须按 ADR-900 流程申请，记录在 ARCH-VIOLATIONS.md
```
...
- [ADR-008 正文](../adr/constitutional/ADR-008-documentation-governance-constitution.md)（权威）
