...
- [ ] 更新 ADR 正文版本号：3.1 → 3.2
      位置：docs/adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md
      
- [ ] 更新架构测试版本号：3.1 → 3.2
      位置：src/tests/ArchitectureTests/ADR/ADR_001_Architecture_Tests.cs
      格式：// Version: 3.2
      
- [ ] 更新 Copilot Prompt 版本号：3.1 → 3.2
      位置：docs/copilot/adr-001.prompts.md
      格式：**版本**：3.2
...
// ADR: ADR-123
public class ADR_123_Architecture_Tests
{
    // ...
}
```
...
```markdown
❓ 问题：在 ADR-001 中新增一个示例代码，版本如何更新？

✅ 答案：小改，版本 3.1 → 3.2
理由：仅补充示例，不改变规则

❓ 问题：在 ADR-001 中新增"禁止跨模块事务"规则，版本如何更新？
...
### 用户问："我修改了 ADR-001（宪法层），其他 ADR 需要更新吗？"
...
```bash
# 查看哪些 ADR 依赖 ADR-001
grep -r "ADR-001" docs/adr/ | grep "依赖"
