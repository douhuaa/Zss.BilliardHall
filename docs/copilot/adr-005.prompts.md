# ADR-005：应用内交互模型与执行边界 - Copilot 提示词库

**关联 ADR**：[ADR-005](../adr/constitutional/ADR-005-Application-Interaction-Model-Final.md)  
**测试类**：`ADR_005_Architecture_Tests.cs`  
**版本**：1.0  
**最后更新**：2026-01-21

---

## 一、当我在写什么时，你应该提醒我哪些 ADR-005 约束？
...
```
根据 ADR-005，Command Handler 不应该返回业务数据。
...
```
根据 ADR-005，Query Handler 不应该修改状态。
...
| ADR 约束      | 测试方法                                                     | 测试内容                                                        |
|-------------|----------------------------------------------------------|-------------------------------------------------------------|
| ADR-005.1  | `Handlers_Should_Have_Clear_Naming_Convention`           | Handler 应有明确的命名约定（CommandHandler/QueryHandler/EventHandler） |
| ADR-005.2  | `Endpoints_Should_Not_Contain_Business_Logic`            | Endpoint 不应包含业务逻辑，注入依赖应少于 5 个                               |
| ADR-005.3  | `Handlers_Should_Not_Depend_On_AspNet`                   | Handler 不应依赖 ASP.NET Core 类型                                |
| ADR-005.4  | `Handlers_Should_Be_Stateless`                           | Handler 应该是无状态的，不包含非 readonly 字段                            |
| ADR-005.5  | `Modules_Should_Not_Have_Synchronous_Cross_Module_Calls` | 模块间不应有未审批的同步调用                                              |
| ADR-005.6  | `Async_Methods_Should_Follow_Naming_Convention`          | 异步方法应以 Async 结尾（Handle 方法除外）                                |
| ADR-005.7  | `Modules_Should_Not_Share_Domain_Entities`               | 模块不应共享领域实体                                                  |
| ADR-005.8  | `QueryHandlers_Can_Return_Contracts`                     | Query Handler 可以返回 Contracts（正面测试）                          |
| ADR-005.9  | `Command_And_Query_Handlers_Should_Be_Separated`         | Command Handler 和 Query Handler 应明确分离                       |
| ADR-005.10 | `CommandHandlers_Should_Not_Return_Business_Data`        | Command Handler 不应返回业务数据，只返回 void 或 ID                      |
| ADR-005.11 | `Handlers_Should_Use_Structured_Exceptions`              | Handler 应使用结构化异常                                            |
| ADR-005.12 | `All_Handlers_Should_Be_In_Module_Assemblies`            | 所有 Handler 应在模块程序集中                                         |
...
```csharp
[Theory(DisplayName = "ADR-005.1: Handler 应有明确的命名约定")]
...
```csharp
[Theory(DisplayName = "ADR-005.10: Command Handler 不应返回业务数据")]
[ClassData(typeof(ModuleAssemblyData))]
    ...
    foreach (var handler in commandHandlers)
    {
        ...
        foreach (var method in handleMethods)
        {
            ...
            if (!allowedTypes.Contains(returnType.Name))
            {
                ...
                {
                    Assert.Fail(
                        $"❌ ADR-005.10 违规: Command Handler 返回业务数据\n\n" +
                        ...
                        $"参考: docs/copilot/adr-005.prompts.md（反模式 2, FAQ Q4）");
                }
            }
        }
    }
...
运行以下命令验证所有 ADR-005 测试：

```bash
# 运行所有 ADR-005 测试
dotnet test --filter "FullyQualifiedName~ADR_005"

# 运行特定约束测试
dotnet test --filter "DisplayName~ADR-005.1"
dotnet test --filter "DisplayName~ADR-005.10"

# 查看详细输出
dotnet test --filter "FullyQualifiedName~ADR_005" --logger "console;verbosity=detailed"
...
2. **添加测试方法**：在 `ADR_005_Architecture_Tests.cs` 中添加对应测试
3. **更新映射表**：
  - 在 ADR-005 文档的"快速参考表"中添加新行
...
- [ADR-005 完整文档](../adr/constitutional/ADR-005-Application-Interaction-Model-Final.md)
- [架构测试类](../../src/tests/ArchitectureTests/ADR/ADR_005_Architecture_Tests.cs)
