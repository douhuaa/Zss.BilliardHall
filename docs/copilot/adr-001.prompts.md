# ADR-001：模块化单体与垂直切片架构 - Copilot 提示词库

**关联 ADR**：[ADR-001](../adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md)  
**测试类**：`ADR_001_Architecture_Tests.cs`  
**版本**：4.0  
**最后更新**：2026-01-23  
**对应 ADR 版本**：v4.0  
**对齐状态**：✅ 100% 完成
...
```
根据 ADR-001，模块内不允许创建横向 Service 层。
...
```
根据 ADR-001，Platform 层不应该包含业务规则或判断。
...
## 四、Contracts 语义约束治理（治理路线图）

### 背景

根据 ADR-001 第 6 节，**Command Handler 不得依赖 Contracts 进行业务决策**，这是宪法层的核心约束之一。

然而，此约束属于**语义级别**，仅通过静态类型依赖检查（如 NetArchTest）无法完全覆盖。例如：

```csharp
// ✅ 类型依赖检查通过（Handler 未直接依赖 Contracts）
// ❌ 但语义违规：通过 Query 获取契约并用于业务决策
public class CreateOrderHandler
{
    public async Task Handle(CreateOrder command)
    {
...
- **执行级别**：L2（语义半自动，需人工评审）
- **检测方式**：
  - NetArchTest 检测类型依赖（已自动化）
  - Code Review 中人工审查语义违规
  - Copilot 通过本 Prompts 文件提供早期提醒

**中期（规划中）**：

- **工具建设**：开发 Roslyn Analyzer 插件
- **检测能力**：
  - 检测 Command Handler 中调用 `IQueryBus.Send<TDto>`
  - 检测契约类型（`Contracts` 命名空间）在业务决策中的使用
  - 提供 IDE 内实时诊断和代码修复建议
- **执行级别**：将升级为 **L1（静态可执行，CI 必须阻断）**

**长期（持续演进）**：

- 扩展 Analyzer 覆盖更多语义约束
- 集成到 CI/CD 流程的更早阶段（如 PR 提交前本地检查）
- 积累最佳实践和反模式库

### 升级条件

**一旦 Roslyn Analyzer 工具就绪并验证稳定，本约束将自动升级为 L1 强制条款：**

- ✅ 工具能准确识别 90% 以上的违规模式
- ✅ 误报率低于 5%
- ✅ 团队完成工具使用培训
- 🚨 升级后，CI 构建将对此类违规直接失败，不接受破例

### 当前最佳实践

在 Roslyn Analyzer 就绪之前，请遵循以下实践：
...
```csharp
// ✅ 正确：Query Handler 可以返回 Contracts
...
- [ADR-905 执行级别分类](../adr/constitutional/ADR-905-enforcement-level-classification.md)
- [ADR-001 第 6 节：Contracts 使用规则](../adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md#6-数据契约contracts使用规则)
...
在审查与 ADR-001 相关的 PR 时，请检查以下映射关系（对应 ADR-001 v3.2）：

| 约束编号       | 描述                | 层级    | 测试方法                                               | 测试文件                           | 状态    |
|------------|-------------------|-------|----------------------------------------------------|--------------------------------|-------|
| ADR-001.1 | 模块不可相互引用          | L1    | `Modules_Should_Not_Reference_Other_Modules`       | ADR_001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-001.2 | 项目文件/程序集禁止引用其他模块  | L1    | `Module_Csproj_Should_Not_Reference_Other_Modules` | ADR_001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-001.3 | 垂直切片/用例为最小单元      | L2    | `Handlers_Should_Be_In_UseCases_Namespace`         | ADR_001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-001.4 | 禁止横向 Service 抽象   | L1    | `Modules_Should_Not_Contain_Service_Classes`       | ADR_001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-001.5 | 只允许事件/契约/原始类型通信   | L2    | `Contract_Rules_Semantic_Check`                    | ADR_001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-001.6 | Contract 不含业务判断字段 | L2/L3 | `Contract_Business_Field_Analyzer`                 | ADR_001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-001.7 | 命名空间/目录强制隔离       | L1    | `Namespace_Should_Match_Module_Boundaries`         | ADR_001_Architecture_Tests.cs | ✅ 已实现 |

**说明**：

- ✅ 已实现：测试已完成且通过验证（13/13 tests passed）
- L1: 静态可执行自动化（ArchitectureTests），CI 直接阻断
...
- 所有 7 项核心约束都有对应测试实现
- 旧的 11 个测试已完全删除，替换为精确的 7 个测试
- 测试代码从 300 行优化到 246 行
- 所有测试通过验证（Passed: 13, Failed: 0）
- 测试方法命名完全符合 ADR-001 v3.2 标准

### 7.2 自检问题

当审查与 ADR-001 相关的代码变更时，询问自己：

1. ✅ **所有标记为【必须架构测试覆盖】的约束是否都有对应的测试方法？**
  - 检查 ADR-001 文档中所有【必须架构测试覆盖】标记
  - 验证是否每条约束都有对应的测试

2. ✅ **测试方法名是否清晰反映了 ADR 约束内容？**
  - 测试方法名应包含 ADR 引用（如 `ADR_001`）
  - DisplayName 应明确说明验证的约束
...
- ✅ DisplayName 明确标注 `ADR-001.1`
- ✅ 测试方法名清晰表达约束内容
- ✅ 失败消息包含 ADR 编号、违规详情和具体修复建议
- ✅ 提供参考文档链接
...
```csharp
/// <summary>
/// ADR-001.4: 验证模块内禁止创建横向 Service 层
/// </summary>
[Theory(DisplayName = "ADR-001.4: 模块不应包含 Service 类")]
[ClassData(typeof(ModuleAssemblyData))]
public void Modules_Should_Not_Contain_Service_Classes(Assembly moduleAssembly)
{
    ...
    Assert.True(result.IsSuccessful,
        $"❌ ADR-001.4 违规: 模块 {moduleAssembly.GetName().Name} 包含 Service 类。\n" +
        $"违规类型: {string.Join(", ", result.FailingTypes?.Select(t => t.FullName))}。\n" +
        $"修复建议：\n" +
        $"  1. 将业务逻辑移到对应的 Handler 中\n" +
        $"  2. 如果是技术性功能，移至 Platform/BuildingBlocks\n" +
        $"  3. 采用垂直切片架构，每个用例有独立的 Handler\n" +
        $"参考：docs/adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md 第 2 节");
}
```

### 7.4 验证脚本

在提交 PR 前，运行 ADR-测试映射验证脚本：
...
4. **验证**：
  - 运行 `./scripts/validate-adr-test-mapping.sh`
...
- [ADR-001 完整文档](../adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md)
- [架构测试类](../../src/tests/ArchitectureTests/ADR/ADR_001_Architecture_Tests.cs)
...
| 版本  | 日期         | 变更说明                                                         |
|-----|------------|--------------------------------------------------------------|
| 1.4 | 2026-01-23 | 完成 v3.2 完整对齐：实现全部 7 项核心约束测试，删除旧测试，所有测试通过验证                   |
| 1.3 | 2026-01-23 | 对齐 ADR-001 v3.2：更新测试映射表，匹配新的 7 项约束编号和执行级别                   |
| 1.2 | 2026-01-23 | 新增"Contracts 语义约束治理（治理路线图）"章节，说明 Roslyn Analyzer 工具规划；调整章节编号 |
| 1.1 | 2026-01-23 | 新增"测试覆盖自检清单"章节，补充 ADR-测试映射要求                                 |
| 1.0 | 2026-01-21 | 初始版本                                                         |
