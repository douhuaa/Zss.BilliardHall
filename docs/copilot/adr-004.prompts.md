# ADR-004：中央包管理（CPM）规范 - Copilot 提示词库

**关联 ADR**：[ADR-004](../adr/constitutional/ADR-004-Cpm-Final.md)  
**测试类**：`ADR_004_Architecture_Tests.cs`  
**版本**：1.0  
**最后更新**：2026-01-21

---

## 一、当我在写什么时，你应该提醒我哪些 ADR-004 约束？
...
```
根据 ADR-004，所有包版本必须在 Directory.Packages.props 中集中管理。
...
```
根据 ADR-004，Platform 层不应该依赖业务框架。
...
```
根据 ADR-004 和 ADR-001，模块之间不允许直接引用。
...
| ADR 约束     | 测试方法                                                            | 测试内容                                        |
|------------|-----------------------------------------------------------------|---------------------------------------------|
| ADR-004.1 | `Repository_Should_Have_Directory_Packages_Props`               | Directory.Packages.props 必须存在于仓库根目录         |
...
| ADR-004.5 | `Directory_Packages_Props_Should_Contain_Package_Groups`        | Directory.Packages.props 应使用 Label 分组       |
| ADR-004.6 | `Directory_Packages_Props_Should_Contain_Common_Package_Groups` | 应包含常见包分组（如 Testing, Logging）                |
| ADR-004.7 | `Platform_Projects_Should_Not_Reference_Business_Packages`      | Platform 不应引用业务包                            |
| ADR-004.8 | `All_Test_Projects_Should_Use_Same_Test_Framework_Versions`     | 所有测试项目应使用相同的测试框架版本                          |
| ADR-004.9 | `Directory_Packages_Props_Should_Define_All_Used_Packages`      | Directory.Packages.props 应定义所有项目使用的包        |
...
```csharp
[Fact(DisplayName = "ADR-004.2: CPM 应被启用")]
public void CPM_Should_Be_Enabled()
{
    ...
    Assert.True(content.Contains("ManagePackageVersionsCentrally"),
        $"❌ ADR-004.2 违规: Directory.Packages.props 必须包含 ManagePackageVersionsCentrally 设置。\n\n" +
    ...
    Assert.True(content.Contains("true"),
        $"❌ ADR-004.2 违规: Directory.Packages.props 中的 ManagePackageVersionsCentrally 应该设置为 true。\n\n" +
...
```csharp
[Fact(DisplayName = "ADR-004.4: 项目文件不应手动指定包版本")]
public void Projects_Should_Not_Specify_Package_Versions()
{
    ...
    Assert.True(violations.Count == 0,
        $"❌ ADR-004.4 违规: 发现 {violations.Count} 个项目手动指定了包版本，应使用 CPM 统一管理。\n\n" +
        $"违规项目:\n{string.Join("\n", violations)}\n\n" +
        $"修复建议:\n" +
...
运行以下命令验证所有 ADR-004 测试：

```bash
# 运行所有 ADR-004 测试
dotnet test --filter "FullyQualifiedName~ADR_004"

# 运行特定约束测试
dotnet test --filter "DisplayName~ADR-004.1"
dotnet test --filter "DisplayName~ADR-004.4"

# 查看详细输出
dotnet test --filter "FullyQualifiedName~ADR_004" --logger "console;verbosity=detailed"
```
...
```
1. 查看失败消息中的 ADR 编号
2. 参考 docs/copilot/adr-004.prompts.md 对应场景或反模式
3. 按照修复建议逐步修复
4. 重新运行测试验证
5. 如有疑问，参考 ADR-004 完整文档
```
...
- [ADR-004 完整文档](../adr/constitutional/ADR-004-Cpm-Final.md)
- [架构测试类](../../src/tests/ArchitectureTests/ADR/ADR_004_Architecture_Tests.cs)
