# ADR-0900：ADR 新增与修订流程 - Copilot 提示词库

**版本**：1.0  
**最后更新**：2026-01-21  
**对应 ADR**：[ADR-0900](../adr/governance/ADR-0900-adr-process.md)
...
根据 ADR-0900，每个新 ADR 必须有架构测试来自动验证约束。
...
根据 ADR-0900，每个新 ADR 必须有 Prompts 文件，以便 Copilot 能够：
- 在开发时提醒约束
- 解释 CI 失败
- 回答开发者问题
...
- **ADR-0000**：定义架构测试要求，ADR-0900 要求每个新 ADR 都有测试
- **所有 ADR**：ADR-0900 定义了所有 ADR 的生命周期管理
...
在审查与 ADR-0900 相关的 PR 时，请检查以下映射关系：

### 映射清单

| ADR 约束                          | 测试方法                                                     | 状态    |
|---------------------------------|----------------------------------------------------------|-------|
| ADR-0900.4: 文档/测试/Prompt 三位一体交付 | `ADR_Documents_Must_Have_Architecture_Tests_And_Prompts` | ✅ 已覆盖 |
| ADR-0900.4: 关键约束必须标记            | `ADR_Documents_Must_Mark_Testable_Constraints`           | ✅ 已覆盖 |
...
public void ADR_Documents_Must_Have_Architecture_Tests_And_Prompts()
{
    ...
    foreach (var adrFile in adrFiles)
    {
        ...
        Assert.True(File.Exists(testFile),
            $"❌ ADR-0900.4 违规: {Path.GetFileName(adrFile)} 缺少架构测试文件。\n" +
            $"修复建议：创建 {testFile}\n" +
            $"参考：docs/copilot/adr-0900.prompts.md");
        
        Assert.True(File.Exists(promptFile),
            $"❌ ADR-0900.4 违规: {Path.GetFileName(adrFile)} 缺少 Copilot Prompts 文件。\n" +
            $"修复建议：创建 {promptFile}\n" +
            $"参考：docs/copilot/adr-0900.prompts.md");
    }
}
