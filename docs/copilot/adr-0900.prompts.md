# ADR-900：ADR 新增与修订流程 - Copilot 提示词库

**版本**：1.2  
**最后更新**：2026-01-21  
**对应 ADR**：[ADR-900](../adr/governance/ADR-900-adr-process.md)
...
根据 ADR-900，每个新 ADR 必须有架构测试来自动验证约束。
...
根据 ADR-900，每个新 ADR 必须有 Prompts 文件，以便 Copilot 能够：
- 在开发时提醒约束
- 解释 CI 失败
- 回答开发者问题
...
- **ADR-0000**：定义架构测试要求，ADR-900 要求每个新 ADR 都有测试
- **所有 ADR**：ADR-900 定义了所有 ADR 的生命周期管理
...
在审查与 ADR-900 相关的 PR 时，请检查以下映射关系：

### 映射清单

| ADR 约束                          | 测试方法                                                     | 状态    |
|---------------------------------|----------------------------------------------------------|-------|
| ADR-900.4: 文档/测试/Prompt 三位一体交付 | `ADR_Documents_Must_Have_Architecture_Tests_And_Prompts` | ✅ 已覆盖 |
| ADR-900.4: 关键约束必须标记            | `ADR_Documents_Must_Mark_Testable_Constraints`           | ✅ 已覆盖 |

### 自检问题

当审查代码变更时，询问自己：

1. ✅ 新增 ADR 是否同时创建了架构测试和 Copilot Prompts？
2. ✅ ADR 文档中的约束是否标记了【必须架构测试覆盖】？
3. ✅ 测试文件命名是否符合 `ADR_XXXX_Architecture_Tests.cs` 格式？
4. ✅ Prompts 文件命名是否符合 `adr-XXXX.prompts.md` 格式？
5. ✅ 是否运行了映射校验脚本（`./scripts/validate-adr-test-mapping.sh`）？
...
```csharp
[Fact(DisplayName = "ADR-900.4: 每个 ADR 必须有对应的架构测试和 Copilot Prompts")]
public void ADR_Documents_Must_Have_Architecture_Tests_And_Prompts()
{
    // 1. 扫描所有 ADR 文档
    var adrFiles = Directory.GetFiles("docs/adr", "ADR-*.md", SearchOption.AllDirectories);
    
    // 2. 检查每个 ADR 是否有对应的测试和 Prompts
    foreach (var adrFile in adrFiles)
    {
        var adrNumber = ExtractAdrNumber(adrFile);
        var testFile = $"src/tests/ArchitectureTests/ADR/ADR_{adrNumber}_Architecture_Tests.cs";
        var promptFile = $"docs/copilot/adr-{adrNumber}.prompts.md";
        
        Assert.True(File.Exists(testFile),
            $"❌ ADR-900.4 违规: {Path.GetFileName(adrFile)} 缺少架构测试文件。\n" +
            $"修复建议：创建 {testFile}\n" +
            $"参考：docs/copilot/adr-0900.prompts.md");
        
        Assert.True(File.Exists(promptFile),
            $"❌ ADR-900.4 违规: {Path.GetFileName(adrFile)} 缺少 Copilot Prompts 文件。\n" +
            $"修复建议：创建 {promptFile}\n" +
            $"参考：docs/copilot/adr-0900.prompts.md");
    }
}
...
- ✅ DisplayName 明确标注 ADR-900.4
- ✅ 失败消息包含 ADR 编号和具体违规文件
- ✅ 提供具体修复建议和参考文档

### 示例 2：验证约束标记

```csharp
[Fact(DisplayName = "ADR-900.4: ADR 文档中的关键约束必须标记为需要测试")]
public void ADR_Documents_Must_Mark_Testable_Constraints()
{
    // 检查有架构测试的 ADR 是否标记了约束
    var adrFiles = GetAdrFilesWithTests();
    
    foreach (var adrFile in adrFiles)
    {
        var content = File.ReadAllText(adrFile);
        var hasMarker = content.Contains("【必须架构测试覆盖】") ||
                       content.Contains("## 快速参考");
        
        // 这是警告级别的检查，不强制失败
        if (!hasMarker)
        {
            Console.WriteLine($"⚠️  {Path.GetFileName(adrFile)} 建议添加约束标记");
        }
    }
}
```
...
| 版本  | 日期         | 变更说明              |
|-----|------------|-------------------|
| 1.1 | 2026-01-23 | 添加测试覆盖自检清单和测试编写示例 |
| 1.0 | 2026-01-21 | 初始版本              |
