...
运行层 ADR（ADR-200~0299）定义了技术实现细节，违反了层级边界。
...
这些属于**技术层（ADR-300~0399）**的职责，不应出现在运行层 ADR 中。
...
| 结构层         | ADR-100~0199   | 结构与组织规范            | 命名规范、DTO 组织      |
| 运行层         | ADR-200~0299   | 运行时行为边界            | 异常传播、事务边界      |
| 技术层         | ADR-300~0399   | 技术选型与实现细节        | 日志框架、数据库选型    |
...
发现以下潜在越权内容：
- "指数退避" → 应移至 ADR-300~0399（技术层）
- "日志字段结构" → 应移至 ADR-300~0399（技术层）
- "重试次数范围" → 应移至 ADR-300~0399（技术层）
