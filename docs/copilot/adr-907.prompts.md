...
> - **Rule**：主要规则（ADR-907_1 ～ ADR-907_4）
> - **Clause**：具体条款（如 ADR-907_2_1, ADR-907_2_2）
...
1. **检查 ADR 编号和测试文件命名**：
   ...
   ✅ 命名格式：
   - 推荐：ADR_907_Architecture_Tests.cs（4位编号）
...
4. **测试技术选项（根据 ADR-907_1_1）**：
   ...
   - 对于 ADR-907_1 自身的元验证测试，建议使用文件系统扫描
   - 复杂规则可混合使用多种技术
   ```
...
参考：
- ADR-907 §2 命名与组织规范
- 现有测试示例：ADR_123_Architecture_Tests.cs
```
...
- 示例：ADR_001_Architecture_Tests.cs
```
...
2. **提供正确的处理方式**：
   ...
   方案 1：修复代码（必须）
   - 按照测试要求调整代码
   - ⚠️ 从 ADR-907 v2.0 起，所有规则均为 L1 级别
   - L1 规则不允许破例，必须修复
   ...
   ❌ 不再支持的方案：
   - 申请破例：ADR-907 已无 L2 规则，不再允许任何破例
   - Skip 测试：会被 ADR-907_2_8 (L1) 检测并阻断
   ```
...
根据 ADR-907 §2.8：
- 禁止使用 Skip 或条件禁用
- Skip 的测试会被 ADR-907_2_8 (L1) 检测出来
...
```
✅ 正确：
- ADR_001_Architecture_Tests.cs（4位，推荐）
...
```
✅ 推荐：
- ADR_001_1_Modules_Should_Not_Reference_Other_Modules
...
| 规则编号 | 执行级别 | 失败处理 |
|---------|---------|---------|
| ADR-907_1_1~1_3 | L1 | 必须修复，不能 Skip |
| ADR-907_2_1~2_8 | L1 | 必须修复，不能 Skip |
| ADR-907_3_1~3_4 | L1 | 必须修复，不能 Skip |
| ADR-907_4_1~4_6 | L1 | 必须修复，不能 Skip |

> ⚠️ **重要变更**：从 ADR-907 v2.0 最新更新起，**所有规则已全部提升为 L1 级别（阻断级）**。
> 不再有 L2 级别的规则，所有违规都会直接阻断 CI。
