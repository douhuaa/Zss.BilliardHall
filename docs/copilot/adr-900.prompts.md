# ADR-900：架构测试与 CI 治理元规则 - Copilot 提示词库

**版本**：2.0  
**最后更新**：2026-01-23  
**对应 ADR**：[ADR-900](../adr/governance/ADR-900-architecture-tests.md)
...
常见架构测试：
- ADR-001: 模块隔离、垂直切片
- ADR-002: 层级依赖、Host 边界
- ADR-003: 命名空间规则
- ADR-004: 中央包管理
- ADR-005: Handler 和 CQRS 模式
...
根据 ADR-900，ADR、测试、Prompts 必须保持一致。
...
```csharp
[Fact(DisplayName = "ADR-001.1: 模块不应相互引用")]
public void Modules_Should_Not_Reference_Other_Modules()
{
    Assert.True(result.IsSuccessful,
...
```
Failed: ADR-001.1: 模块不应相互引用
❌ ADR-001.1 违规: 模块 Orders 不应依赖模块 Members。
违规类型: OrderService
修复建议：使用领域事件或数据契约进行模块间通信。
```
...
- 示例代码: docs/copilot/adr-001.prompts.md
```
...
ADR-010 有 3 条必须测试的约束，但未发现任何测试方法
```
...
3. 快速参考表
   | 约束 | 测试覆盖 |
   |------|---------|
   | 模块隔离 | ✅ ADR_001_Tests::Modules_Should_Not_Reference |
```
...
参考：docs/adr/constitutional/ADR-905-enforcement-level-classification.md
...
| ADR 约束                      | 测试方法                                                     | 状态    |
|-----------------------------|----------------------------------------------------------|-------|
| ADR-900: 每条 ADR 必须有唯一测试类   | `Each_ADR_Must_Have_Exact_And_Unique_Architecture_Test`  | ✅ 已覆盖 |
| ADR-900: 测试类必须包含最少断言数      | `Architecture_Test_Classes_Must_Have_Minimum_Assertions` | ✅ 已覆盖 |
| ADR-900: 测试失败消息必须包含 ADR 编号 | `Test_Failure_Messages_Must_Contain_ADR_Reference`       | ✅ 已覆盖 |
| ADR-900: 禁止跳过架构测试          | `Architecture_Tests_Should_Not_Be_Skipped`               | ✅ 已覆盖 |
...
/// </summary>
[Fact(DisplayName = "ADR-900: 每条 ADR 必须有且仅有唯一对应的架构测试类")]
public void Each_ADR_Must_Have_Exact_And_Unique_Architecture_Test()
{
    ...
    foreach (var adrFile in adrFiles)
    {
        ...
        Assert.Single(matchingTests,
            $"❌ ADR-900 违规: {adrFile} 必须有且仅有一个对应的测试类 {expectedTestType}。\n" +
...
```csharp
[Fact(DisplayName = "ADR-900: 禁止跳过架构测试（反作弊）")]
...
| 版本  | 日期         | 变更说明                     |
|-----|------------|--------------------------|
| 1.0 | 2026-01-23 | 初始版本，基于 ADR-900 v2.0 创建 |
