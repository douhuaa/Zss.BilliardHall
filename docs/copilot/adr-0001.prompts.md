...
| ADR 约束 | 测试方法 | 测试文件 | 状态 |
|---------|---------|---------|------|
| ADR-0001.1: 模块不得相互引用 | `Modules_Should_Not_Reference_Other_Modules` | ADR_0001_Architecture_Tests.cs | ✅ 已覆盖 |
| ADR-0001.2: 模块项目不得引用其他模块 | `Module_Csproj_Should_Not_Reference_Other_Modules` | ADR_0001_Architecture_Tests.cs | ✅ 已覆盖 |
| ADR-0001.3: 禁止传统分层命名空间 | `Modules_Should_Not_Contain_Traditional_Layering_Namespaces` | ADR_0001_Architecture_Tests.cs | ✅ 已覆盖 |
| ADR-0001.4: 禁止横向 Service 类 | `Modules_Should_Not_Contain_Service_Classes` | ADR_0001_Architecture_Tests.cs | ✅ 已覆盖 |
| ADR-0001.5: Handler 必须在 UseCases 下 | `Handlers_Should_Be_In_UseCases_Namespace` | ADR_0001_Architecture_Tests.cs | ✅ 已覆盖 |
| ADR-0001.6: 跨模块禁止引用实体 | 部分覆盖（模块隔离测试） | ADR_0001_Architecture_Tests.cs | 🔄 需补充 |
