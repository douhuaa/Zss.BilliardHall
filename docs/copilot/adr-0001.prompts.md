...
根据 ADR-0001 第 6 节，**Command Handler 不得依赖 Contracts 进行业务决策**，这是宪法层的核心约束之一。
...
- [ADR-0001 第 6 节：Contracts 使用规则](../adr/constitutional/ADR-0001-modular-monolith-vertical-slice-architecture.md#6-数据契约contracts使用规则)
...
在审查与 ADR-0001 相关的 PR 时，请检查以下映射关系（对应 ADR-0001 v3.2）：

| 约束编号       | 描述                | 层级    | 测试方法                                               | 测试文件                           | 状态    |
|------------|-------------------|-------|----------------------------------------------------|--------------------------------|-------|
| ADR-0001.1 | 模块不可相互引用          | L1    | `Modules_Should_Not_Reference_Other_Modules`       | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-0001.2 | 项目文件/程序集禁止引用其他模块  | L1    | `Module_Csproj_Should_Not_Reference_Other_Modules` | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-0001.3 | 垂直切片/用例为最小单元      | L2    | `Handlers_Should_Be_In_UseCases_Namespace`         | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-0001.4 | 禁止横向 Service 抽象   | L1    | `Modules_Should_Not_Contain_Service_Classes`       | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-0001.5 | 只允许事件/契约/原始类型通信   | L2    | `Contract_Rules_Semantic_Check`                    | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-0001.6 | Contract 不含业务判断字段 | L2/L3 | `Contract_Business_Field_Analyzer`                 | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
| ADR-0001.7 | 命名空间/目录强制隔离       | L1    | `Namespace_Should_Match_Module_Boundaries`         | ADR_0001_Architecture_Tests.cs | ✅ 已实现 |
...
- 测试方法命名完全符合 ADR-0001 v3.2 标准

### 7.2 自检问题

当审查与 ADR-0001 相关的代码变更时，询问自己：

1. ✅ **所有标记为【必须架构测试覆盖】的约束是否都有对应的测试方法？**
  - 检查 ADR-0001 文档中所有【必须架构测试覆盖】标记
  - 验证是否每条约束都有对应的测试

2. ✅ **测试方法名是否清晰反映了 ADR 约束内容？**
  - 测试方法名应包含 ADR 引用（如 `ADR_0001`）
...
```csharp
/// <summary>
/// ADR-0001.4: 验证模块内禁止创建横向 Service 层
/// </summary>
[Theory(DisplayName = "ADR-0001.4: 模块不应包含 Service 类")]
[ClassData(typeof(ModuleAssemblyData))]
public void Modules_Should_Not_Contain_Service_Classes(Assembly moduleAssembly)
{
    ...
    Assert.True(result.IsSuccessful,
        $"❌ ADR-0001.4 违规: 模块 {moduleAssembly.GetName().Name} 包含 Service 类。\n" +
        $"违规类型: {string.Join(", ", result.FailingTypes?.Select(t => t.FullName))}。\n" +
        $"修复建议：\n" +
        $"  1. 将业务逻辑移到对应的 Handler 中\n" +
        $"  2. 如果是技术性功能，移至 Platform/BuildingBlocks\n" +
        $"  3. 采用垂直切片架构，每个用例有独立的 Handler\n" +
        $"参考：docs/adr/constitutional/ADR-0001-modular-monolith-vertical-slice-architecture.md 第 2 节");
}
```
...
| 1.3 | 2026-01-23 | 对齐 ADR-0001 v3.2：更新测试映射表，匹配新的 7 项约束编号和执行级别                   |
