# ADR-007 Copilot Prompts：Agent 行为与权限

> ⚖️ **权威声明**：本文件仅为辅助理解 ADR-007 的场景化示例，**不具备裁决权**。
>
> **所有架构判定必须基于 [ADR-007 正文](../adr/constitutional/ADR-007-agent-behavior-permissions-constitution.md)。**

**版本**：1.0  
**对应 ADR**：ADR-007（Agent 行为与权限宪法）  
**最后更新**：2026-01-25
...
- 架构裁决（必须参考 ADR-007 正文）
- 架构测试失败诊断（应参考具体 ADR）
- Agent 行为边界的最终判定（以 ADR-007 为准）

---

## 二、核心概念速查

### Agent 的根本定位（ADR-007 决策 1）
...
- 在 ADR 未明确的情况下给出建议
- 建议绕过 ADR 直接依赖测试
- 违反 ADR-007.2（禁止解释性扩权）和 ADR-007.4（禁止模糊输出）
...
**建议行动**：
1. 查阅 ADR-001 和 ADR-005 的完整正文，确认是否有相关条款
...
**违反的 ADR**：ADR-001 第 3 章"模块隔离与通信规则"第 3.2 节
...
**参考资料**：
- docs/adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md（第 3 章）
- docs/copilot/adr-001.prompts.md（场景 3：跨模块通信）
```
...
**问题**：
- Agent 批准了破例（违反 ADR-007.3：禁止替代性裁决）
- Agent 没有引导正式破例流程
...
根据 ADR-007 决策 3（禁止 2：替代性裁决），Agent 不得批准架构破例。
...
- docs/governance/arch-violations.md（破例记录）
```
...
开发者发现 `adr-001.prompts.md` 中的某个示例与 `ADR-001.md` 正文描述不一致。
...
**冲突说明**：
您发现的冲突是正确的。根据 ADR-007 决策 4，Prompts 文件仅为示例，不具备裁决权。
...
**下一步**：
1. 请提交 PR 修正 `docs/copilot/adr-001.prompts.md`
2. 在 PR 中说明发现的冲突
3. 引用 ADR-001 正文的具体章节

**权威来源**：
- docs/adr/constitutional/ADR-001-modular-monolith-vertical-slice-architecture.md（唯一裁决源）
...
**参考资料**：
- docs/adr/constitutional/ADR-007-agent-behavior-permissions-constitution.md（决策 6）
...
### 禁止 1：解释性扩权（ADR-007.2）

#### ❌ 错误示例

```markdown
虽然 ADR-001 没有明确说明这个场景，但根据我对架构原则的理解，
...
### 禁止 4：Prompts 作为裁决依据（ADR-007.5）

#### ❌ 错误示例

```markdown
根据 adr-001.prompts.md 的场景 3，这是不允许的。
```
...
```markdown
根据 ADR-001 第 3 章"模块隔离与通信规则"，这是不允许的。

参考 adr-001.prompts.md 场景 3 了解示例。
```

---

### 禁止 5：发明架构规则（ADR-007.6）
...
```markdown
根据 ADR-002 第 4 章"依赖注入规范"，你应该使用依赖注入。
```
...
ADR 未明确规定依赖注入的使用方式。建议查阅 ADR-002 或咨询架构师。
```
...
**场景**：adr-001.prompts.md 增加新的示例场景
...
Agent 的响应是否包含三态标识？
├─ 是 → 好，继续
└─ 否 → ⚠️ Agent 违反 ADR-007.1，请报告

响应状态是什么？
├─ ✅ Allowed → 检查是否有 ADR 依据
│   ├─ 有 → 可以继续
│   └─ 无 → ⚠️ Agent 违反 ADR-007.6
│
├─ ⚠️ Blocked → 检查修复方案
│   └─ 按建议修复并验证
│
...
**A:** 以 ADR 正文为准（ADR-007 决策 4）。
- Prompts = 示例，不是规则
- 发现冲突时，修正 Prompts 文件

### Q: Agent 给了模糊建议（"应该可以"），我该怎么办？

**A:** Agent 违反了 ADR-007.4（禁止模糊输出）。
- 要求 Agent 给出明确的三态判断
...
- [ADR-007：Agent 行为与权限宪法](../adr/constitutional/ADR-007-agent-behavior-permissions-constitution.md)（唯一裁决源）

### 辅助

- [Architecture Guardian 配置](../../.github/agents/architecture-guardian.agent.md)（ADR-007 的实例化实现）
...
| 版本  | 日期         | 变更说明           | 修订人            |
|-----|------------|----------------|-----------------|
| 1.0 | 2026-01-25 | 初始版本，提供 ADR-007 场景化示例 | GitHub Copilot  |
