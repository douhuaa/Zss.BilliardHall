...
#### ~~ADR-947_3_1（旧）：ADR 编号使用约束~~ - 已废除
- **状态**：规则已于 2026-02-05 废除（ADR-947 v3.0）
- **原问题**：38个 ADR 文档在非关系声明区包含具体 ADR 编号
- **废除原因**：规则过于严格，禁止了正文中合理的 ADR 引用，影响文档可读性
- **后续影响**：此规则及对应的测试已移除，不再作为架构约束

#### ~~ADR-947_4_1：禁止同编号多文档~~ - 已废除
- **状态**：规则已于 2026-02-05 废除（ADR-947 v3.0）
- **废除原因**：此约束在实践中过于严格，不适合当前文档组织方式
...
### 改进建议
1. ~~**规则合理性评估**：部分规则（如 ADR-947_3_1）过于严格，需要重新评估~~ - 已完成，规则已废除
