...
### 任务1.2：检查并修复ADR-005自引用

**问题**：检测到ADR-005自循环，需确认是否为文档错误
...
### 循环2.1：ADR-001 ↔ ADR-006

**循环**：
- ADR-001（核心架构）依赖 ADR-006（术语宪法）
- ADR-006（术语宪法）依赖 ADR-001（术语参考）

**分析**：
- ADR-001需要术语定义来描述架构
- ADR-006需要引用核心架构来解释术语

**解决方案**（选择一个）：

**方案A**：改为"相关"关系（推荐）
- 将ADR-006对ADR-001的依赖改为"相关"
- 理由：术语宪法并不真正"依赖"核心架构，只是相关

**方案B**：单向依赖
- 只保留ADR-001依赖ADR-006
- 移除ADR-006对ADR-001的依赖
- 理由：术语是基础，架构基于术语

**方案C**：提取公共术语
- 创建ADR-006.1：基础术语（无依赖）
- ADR-001和ADR-006都依赖ADR-006.1

### 循环2.2：ADR-008 ↔ ADR-900

**循环**：
- ADR-008（文档宪法）依赖 ADR-900（ADR流程）
- ADR-900（ADR流程）依赖 ADR-008（文档规范）

**解决方案**（推荐）：
- 审查是否为真实依赖
- 可能将一方改为"相关"关系
- 或明确依赖方向：ADR-900依赖ADR-008（流程基于规范）

### 循环2.3：ADR-900 ↔ ADR-980
