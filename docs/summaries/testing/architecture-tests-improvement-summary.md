...
基于对架构测试体系的深度评审，识别出 3 个关键问题和 3 个前瞻性改进方向。本次改进旨在将架构测试从"合格且罕见地成熟"提升到"可以长期守住的宪法级体系"。
...
1. **创建了执行级别分类文档** (`docs/adr/ADR-0005-Enforcement-Levels.md`)
...
创建了 `docs/adr/ADR-0005-Enforcement-Levels.md` 文档，明确定义：
...
2. **创建了破例记录表** ([`docs/summaries/governance/arch-violations.md`](../../summaries/governance/arch-violations.md))
   - 活跃破例（尚未归还）
   - 已归还破例（历史审计）
   - 永久破例（需全体批准）
   - 被拒绝申请（决策参考）
...
3. `docs/adr/ADR-0005-Enforcement-Levels.md` - 执行级别分类（183 行）
...
2. **如需申请破例**
   ...
   - 等待架构师审批
   - 破例批准后会被记录在 [`arch-violations.md`](../../summaries/governance/arch-violations.md)
...
2. **批准破例**
   - 根据影响时长选择审批级别
   - 在 [`arch-violations.md`](../../summaries/governance/arch-violations.md) 中记录破例
   - 设置提醒，跟踪归还进度

3. **定期审计**
   - 每季度检查 [`arch-violations.md`](../../summaries/governance/arch-violations.md)
   - 核实已归还的破例是否真正修复
   - 评估破例趋势，决定是否需要修订 ADR
...
2. **团队培训**
   - 新成员入职时讲解架构宪法层
   - 定期回顾 [`arch-violations.md`](../../summaries/governance/arch-violations.md)
   - 分享典型的拒绝案例和成功案例
...
2. **自动化审计**
   - 定期扫描 [`arch-violations.md`](../../summaries/governance/arch-violations.md)
   - 自动提醒超期未归还的破例
   - 生成季度审计报告
