...
在优化前，项目已建立了宪法层（ADR-900~0005）架构治理体系，但存在以下问题：
...
| **治理层** | `ADR-900, 900~999` | `governance/`     | 治理/破例/流程/变更管理 | ✅ 2个 ADR |

**特殊编号处理**：

- `ADR-900`：架构测试元规则，保持原编号（治理层核心，特殊地位）
- `ADR-900`：ADR 流程管理（原 ADR-900A，重新编号）
...
- ✅ 修改 ADR_900_Architecture_Tests.cs
  - 支持递归搜索子目录中的 ADR 文件
