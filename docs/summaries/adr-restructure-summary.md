...
| ADR      | 优化前主要内容                   | 优化后聚焦内容                |
|----------|---------------------------|------------------------|
| ADR-001 | 模块组织 + 垂直切片 + 架构测试 + 命名空间 | **仅**模块组织与垂直切片         |
| ADR-002 | 启动体系 + 命名空间 + 架构测试        | **仅**启动体系与层级模型         |
| ADR-003 | 命名空间 + 架构测试               | **仅**命名空间与工程映射         |
| ADR-004 | 包管理 + 架构测试                | **仅**中央包管理与依赖分层        |
| ADR-005 | 运行时模型 + 架构测试 + 静态组织       | **仅**应用内运行模型（移除静态组织内容） |
| ADR-900 | 架构测试基本定义                  | **汇总**所有架构测试与治理机制      |
...
- **ADR-001**：模块如何划分，如何组织
- **ADR-002**：系统如何启动，层级如何装配
- **ADR-003**：命名空间如何映射，如何推导
- **ADR-004**：依赖包如何管理，如何分层

#### 运行时行为层（HOW）- 回答"系统如何运行"

- **ADR-005**：用例如何执行，模块如何通信，一致性如何保障

#### 架构治理层（GUARDRAIL）- 回答"如何保障架构不腐化"

- **ADR-900**：如何自动化测试，如何 CI 阻断，如何处理破例
...
1. 查看 ADR-001（模块隔离规则）
2. 查看 ADR-005（应用内交互模型）
3. 可能还需要查看 ADR-900（架构测试）
4. 内容分散，需要自己综合

#### 优化后

1. 在 README.md 中查询"模块如何通信"
2. 直接跳转到 ADR-001（静态约束）和 ADR-005（运行时行为）
...
1. ADR-002 描述启动体系
2. 但也包含命名空间规则（应该在 ADR-003）
3. 还包含架构测试（应该在 ADR-900）
4. 内容混杂，难以聚焦
...
3. "与其他 ADR 关系"明确引用 ADR-003（命名空间）和 ADR-900（测试）
4. 内容聚焦，边界清晰
...
|----------------------------------------------------------|-------|------|
| README.md                                                | 索引    | ✅ 新增 |
| ADR-900-architecture-tests.md                           | 治理层   | ✅ 增强 |
| ADR-001-modular-monolith-vertical-slice-architecture.md | 静态结构层 | ✅ 优化 |
| ADR-002-platform-application-host-bootstrap.md          | 静态结构层 | ✅ 优化 |
| ADR-003-namespace-rules.md                              | 静态结构层 | ✅ 优化 |
| ADR-004-Cpm-Final.md                                    | 静态结构层 | ✅ 优化 |
| ADR-005-Application-Interaction-Model-Final.md          | 运行时层  | ✅ 优化 |
