...
**审计依据**：[ADR-006 术语与编号宪法](../docs/adr/constitutional/ADR-006-terminology-numbering-constitution.md)  
**审计日期**：2026-01-25  
...
根据 ADR-006 的编号规则和语义约束，对现有 ADR 进行了全面审计。
...
- 根据 ADR-006，治理层允许包含流程 + 工具 + 人的混合规则
...
|----------|-----|--------------------------------------------|---------|-----|
| ADR-001 | 宪法层 | `ADR-001-*.md`                            | ✅ 使用前导零 | ✅   |
| ADR-002 | 宪法层 | `ADR-002-*.md`                            | ✅ 使用前导零 | ✅   |
| ADR-003 | 宪法层 | `ADR-003-*.md`                            | ✅ 使用前导零 | ✅   |
| ADR-004 | 宪法层 | `ADR-004-*.md`                            | ✅ 使用前导零 | ✅   |
| ADR-005 | 宪法层 | `ADR-005-*.md`                            | ✅ 使用前导零 | ✅   |
| ADR-006 | 宪法层 | `ADR-006-*.md`                            | ✅ 使用前导零 | ✅   |
...
根据 ADR-006 规则，治理层应不使用前导零（如 ADR-930）。
...
| 编号段              | 应归属目录           | 实际情况                | 合规性 |
|------------------|-----------------|---------------------|-----|
| ADR-001~0009    | constitutional/ | ✅ 所有 6 个 ADR 均在此目录 | ✅   |
...
1. 根据 ADR-006，治理层 ADR 的定义是"治理、流程、破例管理"
...
1. ADR-006 第 3 节明确说明 ADR-900 的特殊地位
...
- 或在 ADR-006 中明确说明 ADR-900 的特殊情况
...
✅ **现有 ADR 编号体系符合 ADR-006 规范，无需迁移或重新编号。**
...
|---------|------------------|-------------------------------|----|-|
| **宪法层** | ADR-001~0009    | 0001, 0002, 0003, 0004, 0005, 0006 | 6  | 0007~0009（3个）|
...
**审计人**：GitHub Copilot  
**审计日期**：2026-01-25  
**审计依据**：ADR-006 术语与编号宪法  
**审计范围**：所有现有 ADR（22 个）  
**审计结论**：✅ 完全合规

**审计声明**：

本审计报告基于 ADR-006 定义的编号规则和语义约束，对所有现有 ADR 进行了全面审计。
...
> **现有 ADR 编号体系符合 ADR-006 规范，无需迁移或重新编号。ADR-006 的引入是"立法"而非"追溯执法"，现有体系已经是合规的。**
...
- [ADR-006 Copilot 提示词](../docs/copilot/adr-006.prompts.md)
