...
| ADR-0000 | 治理层 | `ADR-0000-*.md`                            | ✅ 特殊编号  | ✅   |
| ADR-0900 | 治理层 | `ADR-0900-*.md`（注意：ADR-0900 使用前导零）       | ⚠️ 特殊情况  | ✅   |
...
ADR-0900 使用前导零（`ADR-0900`）是**历史原因**（原编号为 ADR-0000A）。
...
- ⚠️ **可选**：ADR-0900 可考虑重命名为 `ADR-900`（需特批迁移）
...
| ADR-0000, 900~999 | governance/     | ✅ 所有 3 个 ADR 均在此目录 | ✅   |
...
### 关注点 2：ADR-0000 的特殊编号

**描述**：

ADR-0000 使用特殊编号（0000），不在标准编号段内。
...
1. ADR-0006 第 3 节明确说明 ADR-0000 的特殊地位
...
ADR-0900 使用前导零，但治理层其他 ADR（如 ADR-930）不使用前导零。
...
1. ADR-0900 原编号为 ADR-0000A，迁移时保留了前导零
2. 0900 本身是一个合法的编号表示
3. 不影响编号段语义
...
- 可考虑将 `ADR-0900` 重命名为 `ADR-900`（需特批迁移）
- 或在 ADR-0006 中明确说明 ADR-0900 的特殊情况
...
5. **特殊编号**：ADR-0000 和 ADR-0900 的特殊情况已在 ADR-0006 中明确

#### ⚠️ 轻微关注点（不违规）

1. **ADR-930 内容混合**：流程 + 工具 + 人员，但这是治理层的合法特征
2. **ADR-0900 前导零**：历史原因，可接受，未来应遵循"不使用前导零"规则
...
| **治理层** | ADR-0000, 900~999 | 0000, 0900, 930               | 3  | 901~929, 931~999（98个） |
