---
title: "支付模块（Payments）"
description: "基于 Wolverine 垂直切片架构的支付流程与对账管理模块设计"
version: "2.0.0"
author: "架构团队"
maintainer: "后端开发团队"
created: "2024-01-15"
updated: "2024-01-15"
category: "模块设计"
level: "核心"
audience: ["后端开发工程师", "支付工程师"]
keywords: ["Payments", "支付", "幂等性", "退款", "Wolverine", "垂直切片"]
tags: ["payments", "gateway", "idempotency", "refund", "vertical-slice", "wolverine"]
dependencies: ["系统架构设计", "Wolverine模块化架构蓝图"]
estimated_reading_time: "22分钟"
architecture: "Wolverine + 垂直切片"
---

# 支付模块（Payments）

<!-- Breadcrumb Navigation -->
**导航路径**: [🏠 项目文档](../../README.md) > [📦 模块设计](README.md) > 💳 支付模块

<!-- Keywords for Search -->
**关键词**: `Payments` `支付模块` `幂等性` `退款管理` `Wolverine` `垂直切片`

---

## 📋 模块概述

支付模块（Payments）负责管理支付流程、对账、退款和支付历史，采用 **Wolverine + 垂直切片架构**。

### 核心职责

- 💳 **支付处理**: 支持多种支付方式（现金/支付宝/微信/会员余额）
- 🔒 **幂等性保证**: 使用幂等性密钥防止重复支付
- 💰 **退款管理**: 退款申请、审核、执行
- 📊 **对账管理**: 日对账、月对账、异常处理
- 📈 **营收报表**: 日营收、月营收、支付方式统计

### 架构参考

> 📚 **完整架构实现**: [Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)
> 
> 📚 **模块边界定义**: [系统模块划分 - Payments 模块](../03_系统架构设计/系统模块划分.md#6-payments-模块支付对账)

---

## 🏗️ 模块结构（垂直切片）

### 目录组织

\`\`\`
src/Modules/Payments/
├── ProcessPayment/
│   ├── ProcessPayment.cs              # Command
│   ├── ProcessPaymentHandler.cs       # Handler
│   ├── ProcessPaymentEndpoint.cs      # HTTP Endpoint
│   └── ProcessPaymentValidator.cs     # Validator
│
├── RefundPayment/
│   ├── RefundPayment.cs
│   ├── RefundPaymentHandler.cs
│   └── RefundPaymentEndpoint.cs
│
├── ReconcilePayments/
│   ├── ReconcilePayments.cs
│   ├── ReconcilePaymentsHandler.cs
│   └── ReconciliationService.cs       # 对账服务
│
├── GetPayment/
│   ├── GetPayment.cs                  # Query
│   ├── GetPaymentHandler.cs
│   └── GetPaymentEndpoint.cs
│
├── GetPaymentHistory/
│   ├── GetPaymentHistory.cs
│   └── GetPaymentHistoryHandler.cs
│
├── GetDailyRevenue/
│   ├── GetDailyRevenue.cs
│   └── GetDailyRevenueHandler.cs
│
├── Events/
│   ├── PaymentCompleted.cs            # 支付完成
│   ├── PaymentFailed.cs               # 支付失败
│   └── PaymentRefunded.cs             # 已退款
│
├── Handlers/
│   └── BillCalculatedHandler.cs       # 监听 BillCalculated 事件 → 创建支付单
│
├── Payment.cs                          # 聚合根
├── PaymentMethod.cs                    # 枚举：支付方式
├── PaymentStatus.cs                    # 枚举：支付状态
└── PaymentsModule.cs                   # 模块标记
\`\`\`

---

## 💻 功能切片实现

### 1. ProcessPayment（处理支付）

#### Command 定义

\`\`\`csharp
public sealed record ProcessPayment(
    Guid BillId,
    Guid SessionId,
    decimal Amount,
    PaymentMethod Method,
    Guid? MemberId,
    string IdempotencyKey           // 幂等性密钥
);
\`\`\`

#### Handler 实现

\`\`\`csharp
public sealed class ProcessPaymentHandler
{
    [Transactional]
    public async Task<Result<Guid>> Handle(
        ProcessPayment command,
        IDocumentSession session,
        IMessageBus bus,
        CancellationToken ct = default)
    {
        // 1. 幂等性检查：检查是否已处理相同的幂等性密钥
        var existingPayment = await session.Query<Payment>()
            .FirstOrDefaultAsync(p => p.IdempotencyKey == command.IdempotencyKey, ct);
        
        if (existingPayment != null)
        {
            if (existingPayment.Status == PaymentStatus.Completed)
                return Result.Ok(existingPayment.Id);  // 已完成，返回原支付ID
            
            if (existingPayment.Status == PaymentStatus.Processing)
                return Result.Fail<Guid>("Payments:PaymentProcessing", "支付正在处理中，请稍后");
        }
        
        // 2. 验证账单
        var bill = await session.LoadAsync<Bill>(command.BillId, ct);
        if (bill == null)
            return Result.Fail<Guid>("Payments:BillNotFound", "账单不存在");
        
        if (bill.TotalAmount != command.Amount)
            return Result.Fail<Guid>("Payments:AmountMismatch", "支付金额与账单不符");
        
        // 3. 创建支付记录
        var payment = new Payment
        {
            Id = Guid.NewGuid(),
            BillId = command.BillId,
            SessionId = command.SessionId,
            MemberId = command.MemberId,
            Amount = command.Amount,
            Method = command.Method,
            Status = PaymentStatus.Processing,
            IdempotencyKey = command.IdempotencyKey,
            CreatedAt = DateTimeOffset.UtcNow
        };
        
        session.Store(payment);
        await session.SaveChangesAsync(ct);
        
        // 4. 处理不同支付方式
        var result = command.Method switch
        {
            PaymentMethod.MemberBalance => await ProcessMemberBalancePayment(
                command.MemberId!.Value, command.Amount, session, bus, ct),
            PaymentMethod.Cash => ProcessCashPayment(),
            PaymentMethod.Alipay => await ProcessAlipayPayment(payment.Id, command.Amount),
            PaymentMethod.WeChatPay => await ProcessWeChatPayment(payment.Id, command.Amount),
            _ => Result.Fail("Payments:UnsupportedMethod", "不支持的支付方式")
        };
        
        if (!result.IsSuccess)
        {
            payment.Status = PaymentStatus.Failed;
            payment.FailureReason = result.Error?.Message;
            session.Store(payment);
            await session.SaveChangesAsync(ct);
            
            await bus.PublishAsync(new PaymentFailed(payment.Id, command.SessionId), ct);
            return Result.Fail<Guid>(result.Error!.Code, result.Error.Message);
        }
        
        // 5. 标记支付完成
        payment.Status = PaymentStatus.Completed;
        payment.CompletedAt = DateTimeOffset.UtcNow;
        payment.TransactionId = result.Value as string;
        
        session.Store(payment);
        await session.SaveChangesAsync(ct);
        
        // 6. 发布支付完成事件
        await bus.PublishAsync(new PaymentCompleted(
            payment.Id,
            command.SessionId,
            command.MemberId,
            command.Amount
        ), ct);
        
        return Result.Ok(payment.Id);
    }
    
    private async Task<Result> ProcessMemberBalancePayment(
        Guid memberId,
        decimal amount,
        IDocumentSession session,
        IMessageBus bus,
        CancellationToken ct)
    {
        // 发送命令到 Members 模块扣减余额
        return await bus.InvokeAsync<Result>(
            new DeductBalance(memberId, amount, "台球消费"),
            ct
        );
    }
    
    private Result ProcessCashPayment()
    {
        // 现金支付直接成功
        return Result.Ok();
    }
    
    private async Task<Result> ProcessAlipayPayment(Guid paymentId, decimal amount)
    {
        // TODO: 调用支付宝 SDK
        await Task.Delay(100);  // 模拟调用
        return Result.Ok($"ALIPAY_{paymentId}");
    }
    
    private async Task<Result> ProcessWeChatPayment(Guid paymentId, decimal amount)
    {
        // TODO: 调用微信支付 SDK
        await Task.Delay(100);  // 模拟调用
        return Result.Ok($"WECHAT_{paymentId}");
    }
}
\`\`\`

#### HTTP Endpoint

\`\`\`csharp
public sealed class ProcessPaymentEndpoint
{
    [WolverinePost("/api/payments/process")]
    public static ProcessPayment Post(ProcessPaymentRequest request, HttpContext httpContext)
    {
        // 从请求头获取幂等性密钥，如果没有则生成
        var idempotencyKey = httpContext.Request.Headers["X-Idempotency-Key"].FirstOrDefault()
            ?? Guid.NewGuid().ToString();
        
        return new ProcessPayment(
            request.BillId,
            request.SessionId,
            request.Amount,
            request.Method,
            request.MemberId,
            idempotencyKey
        );
    }
}

public sealed record ProcessPaymentRequest(
    Guid BillId,
    Guid SessionId,
    decimal Amount,
    PaymentMethod Method,
    Guid? MemberId
);
\`\`\`

#### Validator

\`\`\`csharp
public sealed class ProcessPaymentValidator : AbstractValidator<ProcessPayment>
{
    public ProcessPaymentValidator()
    {
        RuleFor(x => x.BillId)
            .NotEmpty().WithMessage("BillId 不能为空");
        
        RuleFor(x => x.SessionId)
            .NotEmpty().WithMessage("SessionId 不能为空");
        
        RuleFor(x => x.Amount)
            .GreaterThan(0).WithMessage("支付金额必须大于0");
        
        RuleFor(x => x.IdempotencyKey)
            .NotEmpty().WithMessage("幂等性密钥不能为空");
        
        When(x => x.Method == PaymentMethod.MemberBalance, () =>
        {
            RuleFor(x => x.MemberId)
                .NotEmpty().WithMessage("使用会员余额支付时必须提供会员ID");
        });
    }
}
\`\`\`

---

### 2. RefundPayment（退款）

#### Command 定义

\`\`\`csharp
public sealed record RefundPayment(
    Guid PaymentId,
    decimal Amount,
    string Reason
);
\`\`\`

#### Handler 实现

\`\`\`csharp
public sealed class RefundPaymentHandler
{
    [Transactional]
    public async Task<Result<Guid>> Handle(
        RefundPayment command,
        IDocumentSession session,
        IMessageBus bus,
        CancellationToken ct = default)
    {
        var payment = await session.LoadAsync<Payment>(command.PaymentId, ct);
        if (payment == null)
            return Result.Fail<Guid>("Payments:PaymentNotFound", "支付记录不存在");
        
        if (payment.Status != PaymentStatus.Completed)
            return Result.Fail<Guid>("Payments:PaymentNotCompleted", "只能退款已完成的支付");
        
        if (payment.Status == PaymentStatus.Refunded)
            return Result.Fail<Guid>("Payments:AlreadyRefunded", "该支付已退款");
        
        if (command.Amount > payment.Amount)
            return Result.Fail<Guid>("Payments:RefundAmountExceeded", "退款金额超过原支付金额");
        
        // 创建退款记录
        var refund = new Refund
        {
            Id = Guid.NewGuid(),
            PaymentId = payment.Id,
            Amount = command.Amount,
            Reason = command.Reason,
            Status = RefundStatus.Processing,
            CreatedAt = DateTimeOffset.UtcNow
        };
        
        // 根据支付方式处理退款
        var refundResult = payment.Method switch
        {
            PaymentMethod.MemberBalance => await RefundToMemberBalance(
                payment.MemberId!.Value, command.Amount, session, bus, ct),
            PaymentMethod.Cash => ProcessCashRefund(),
            PaymentMethod.Alipay => await ProcessAlipayRefund(payment.TransactionId!, command.Amount),
            PaymentMethod.WeChatPay => await ProcessWeChatRefund(payment.TransactionId!, command.Amount),
            _ => Result.Fail("Payments:UnsupportedRefundMethod", "不支持的退款方式")
        };
        
        if (!refundResult.IsSuccess)
        {
            refund.Status = RefundStatus.Failed;
            refund.FailureReason = refundResult.Error?.Message;
        }
        else
        {
            refund.Status = RefundStatus.Completed;
            refund.CompletedAt = DateTimeOffset.UtcNow;
            payment.Status = PaymentStatus.Refunded;
            payment.RefundedAt = DateTimeOffset.UtcNow;
        }
        
        session.Store(refund);
        session.Store(payment);
        await session.SaveChangesAsync(ct);
        
        await bus.PublishAsync(new PaymentRefunded(payment.Id, refund.Id, command.Amount), ct);
        
        return Result.Ok(refund.Id);
    }
    
    private async Task<Result> RefundToMemberBalance(
        Guid memberId,
        decimal amount,
        IDocumentSession session,
        IMessageBus bus,
        CancellationToken ct)
    {
        return await bus.InvokeAsync<Result>(
            new TopUpBalance(memberId, amount, "退款"),
            ct
        );
    }
    
    private Result ProcessCashRefund()
    {
        return Result.Ok();
    }
    
    private async Task<Result> ProcessAlipayRefund(string transactionId, decimal amount)
    {
        // TODO: 调用支付宝退款 API
        await Task.Delay(100);
        return Result.Ok();
    }
    
    private async Task<Result> ProcessWeChatRefund(string transactionId, decimal amount)
    {
        // TODO: 调用微信退款 API
        await Task.Delay(100);
        return Result.Ok();
    }
}
\`\`\`

---

### 3. BillCalculatedHandler（监听账单计算事件）

\`\`\`csharp
public sealed class BillCalculatedHandler
{
    public async Task Handle(
        BillCalculated @event,
        IDocumentSession session,
        CancellationToken ct = default)
    {
        // 账单生成后，创建待支付记录
        var paymentRecord = new PaymentRecord
        {
            Id = Guid.NewGuid(),
            BillId = @event.BillId,
            SessionId = @event.SessionId,
            Amount = @event.TotalAmount,
            Status = PaymentRecordStatus.Pending,
            CreatedAt = DateTimeOffset.UtcNow
        };
        
        session.Store(paymentRecord);
        await session.SaveChangesAsync(ct);
    }
}
\`\`\`

---

## 📊 领域模型

### Payment（聚合根）

\`\`\`csharp
public class Payment
{
    public Guid Id { get; set; }
    public Guid BillId { get; set; }
    public Guid SessionId { get; set; }
    public Guid? MemberId { get; set; }
    public decimal Amount { get; set; }
    public PaymentMethod Method { get; set; }
    public PaymentStatus Status { get; set; }
    public string? TransactionId { get; set; }      // 第三方交易号
    public string IdempotencyKey { get; set; } = string.Empty;
    public string? FailureReason { get; set; }
    public DateTimeOffset CreatedAt { get; set; }
    public DateTimeOffset? CompletedAt { get; set; }
    public DateTimeOffset? RefundedAt { get; set; }
}

public enum PaymentMethod
{
    Cash,           // 现金
    Alipay,         // 支付宝
    WeChatPay,      // 微信支付
    MemberBalance,  // 会员余额
    BankCard        // 银行卡
}

public enum PaymentStatus
{
    Pending,    // 待支付
    Processing, // 处理中
    Completed,  // 已完成
    Failed,     // 失败
    Refunded    // 已退款
}
\`\`\`

### Refund（退款记录）

\`\`\`csharp
public class Refund
{
    public Guid Id { get; set; }
    public Guid PaymentId { get; set; }
    public decimal Amount { get; set; }
    public string Reason { get; set; } = string.Empty;
    public RefundStatus Status { get; set; }
    public string? FailureReason { get; set; }
    public DateTimeOffset CreatedAt { get; set; }
    public DateTimeOffset? CompletedAt { get; set; }
}

public enum RefundStatus
{
    Processing,  // 处理中
    Completed,   // 已完成
    Failed       // 失败
}
\`\`\`

---

## 🔔 领域事件

\`\`\`csharp
public sealed record PaymentCompleted(
    Guid PaymentId,
    Guid SessionId,
    Guid? MemberId,
    decimal Amount
);

public sealed record PaymentFailed(
    Guid PaymentId,
    Guid SessionId
);

public sealed record PaymentRefunded(
    Guid PaymentId,
    Guid RefundId,
    decimal Amount
);
\`\`\`

---

## 🔗 跨模块通信

**监听事件**:
- `BillCalculated` (来自 Billing 模块) → 创建待支付记录

**发送命令**:
- → `DeductBalance` (到 Members 模块) - 使用会员余额支付
- → `TopUpBalance` (到 Members 模块) - 退款至会员余额

**发布事件**:
- `PaymentCompleted` → Sessions 模块监听（标记时段完成）
- `PaymentCompleted` → Members 模块监听（赠送积分）

---

## 🧪 单元测试示例

### ProcessPaymentHandler 测试

\`\`\`csharp
public class ProcessPaymentHandlerTests
{
    [Fact]
    public async Task Handle_CashPayment_Succeeds()
    {
        // Arrange
        var store = DocumentStore.For(opts =>
        {
            opts.Connection("connection string");
            opts.DatabaseSchemaName = "payments_test";
        });
        
        await using var session = store.LightweightSession();
        var bus = Substitute.For<IMessageBus>();
        
        var bill = new Bill
        {
            Id = Guid.NewGuid(),
            SessionId = Guid.NewGuid(),
            TotalAmount = 50m
        };
        session.Store(bill);
        await session.SaveChangesAsync();
        
        var handler = new ProcessPaymentHandler();
        var command = new ProcessPayment(
            bill.Id,
            bill.SessionId,
            50m,
            PaymentMethod.Cash,
            null,
            Guid.NewGuid().ToString()
        );
        
        // Act
        var result = await handler.Handle(command, session, bus, CancellationToken.None);
        
        // Assert
        Assert.True(result.IsSuccess);
        
        var payment = await session.LoadAsync<Payment>(result.Value);
        Assert.NotNull(payment);
        Assert.Equal(PaymentStatus.Completed, payment.Status);
        Assert.Equal(50m, payment.Amount);
        
        await bus.Received(1).PublishAsync(
            Arg.Is<PaymentCompleted>(e => e.PaymentId == result.Value),
            Arg.Any<CancellationToken>()
        );
    }
    
    [Fact]
    public async Task Handle_IdempotencyCheck_ReturnsSamePaymentId()
    {
        // Arrange
        var store = DocumentStore.For(opts =>
        {
            opts.Connection("connection string");
            opts.DatabaseSchemaName = "payments_test";
        });
        
        await using var session = store.LightweightSession();
        var bus = Substitute.For<IMessageBus>();
        
        var idempotencyKey = Guid.NewGuid().ToString();
        var existingPayment = new Payment
        {
            Id = Guid.NewGuid(),
            BillId = Guid.NewGuid(),
            SessionId = Guid.NewGuid(),
            Amount = 50m,
            Method = PaymentMethod.Cash,
            Status = PaymentStatus.Completed,
            IdempotencyKey = idempotencyKey,
            CreatedAt = DateTimeOffset.UtcNow,
            CompletedAt = DateTimeOffset.UtcNow
        };
        session.Store(existingPayment);
        await session.SaveChangesAsync();
        
        var handler = new ProcessPaymentHandler();
        var command = new ProcessPayment(
            existingPayment.BillId,
            existingPayment.SessionId,
            50m,
            PaymentMethod.Cash,
            null,
            idempotencyKey  // 使用相同的幂等性密钥
        );
        
        // Act
        var result = await handler.Handle(command, session, bus, CancellationToken.None);
        
        // Assert
        Assert.True(result.IsSuccess);
        Assert.Equal(existingPayment.Id, result.Value);  // 返回原支付ID
        
        // 不应发布新的事件
        await bus.DidNotReceive().PublishAsync(
            Arg.Any<PaymentCompleted>(),
            Arg.Any<CancellationToken>()
        );
    }
}
\`\`\`

---

## 📝 开发规范

### 命名约定

- **Command**: 动词开头（`ProcessPayment`, `RefundPayment`）
- **Query**: `Get` 开头（`GetPayment`, `GetPaymentHistory`）
- **Event**: 过去式（`PaymentCompleted`, `PaymentRefunded`）

### 幂等性实现

**关键点**:
1. 每个支付请求必须携带唯一的幂等性密钥
2. 处理前检查是否已存在相同密钥的支付记录
3. 如果已完成，直接返回原支付ID
4. 如果正在处理，返回"处理中"错误

**Wolverine 配置**:
\`\`\`csharp
opts.Policies.Add<RetryPolicy>(policy =>
{
    policy.MaxAttempts = 3;
    policy.Exceptions.Add<HttpRequestException>();
    policy.Exceptions.Add<TimeoutException>();
});
\`\`\`

### 日志规范

\`\`\`csharp
_logger.LogInformation(
    "Payment {PaymentId} for Session {SessionId} completed with {Method}, Amount: {Amount:F2}",
    paymentId, sessionId, method, amount
);

_logger.LogWarning(
    "Payment {PaymentId} failed: {Reason}",
    paymentId, failureReason
);
\`\`\`

### 错误处理

- 错误代码格式：`Payments:{ErrorKey}`
- 示例：`Payments:AmountMismatch`, `Payments:PaymentProcessing`

---

## 📚 相关文档

- [Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)
- [系统模块划分](../03_系统架构设计/系统模块划分.md)
- [台球桌计费模块](台球桌计费模块.md)
- [会员管理模块](会员管理模块.md)

---

*最后更新: 2024-01-15 | 版本: v2.0.0 | 架构: Wolverine + 垂直切片*
