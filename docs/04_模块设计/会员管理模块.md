---
title: "会员管理模块（Members）"
description: "基于 Wolverine 垂直切片架构的会员管理模块设计"
version: "3.0.0"
author: "架构团队"
maintainer: "后端开发团队"
created: "2024-01-15"
updated: "2024-01-15"
category: "模块设计"
level: "核心"
audience: ["后端开发工程师", "业务分析师"]
keywords: ["会员管理", "Members", "Wolverine", "垂直切片", "CQRS"]
tags: ["member", "user", "points", "authentication", "vertical-slice", "wolverine"]
dependencies: ["系统架构设计", "Wolverine模块化架构蓝图"]
estimated_reading_time: "20分钟"
architecture: "Wolverine + 垂直切片"
---

# 会员管理模块（Members）

<!-- Breadcrumb Navigation -->
**导航路径**: [🏠 项目文档](../../README.md) > [📦 模块设计](README.md) > 🏃 会员管理模块

<!-- Keywords for Search -->
**关键词**: `Members` `会员管理` `用户系统` `积分体系` `Wolverine` `垂直切片`

---

## 📋 模块概述

会员管理模块（Members）是自助台球系统的核心用户模块，采用 **Wolverine + 垂直切片架构**。负责会员注册、资料管理、余额充值、积分管理、等级体系等功能。

### 核心职责

- 🔐 **会员注册与认证**: 注册新会员、OpenIddict 认证集成
- 💰 **余额管理**: 充值、扣减、余额查询
- 🎯 **积分系统**: 积分获取、消费、兑换管理
- 🏆 **会员等级体系**: 等级划分、权益管理、自动升级
- 👤 **资料管理**: 更新资料、查询会员信息

### 架构参考

> 📚 **完整架构实现**: [Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)
> 
> 📚 **模块边界定义**: [系统模块划分 - Members 模块](../03_系统架构设计/系统模块划分.md#7-members-模块会员体系)

---

## 🏗️ 模块结构（垂直切片）

### 目录组织

```
src/Modules/Members/
├── RegisterMember/
│   ├── RegisterMember.cs              # Command 定义
│   ├── RegisterMemberHandler.cs       # Handler 实现
│   ├── RegisterMemberEndpoint.cs      # HTTP 端点
│   └── RegisterMemberValidator.cs     # 验证器（FluentValidation）
│
├── UpdateMemberProfile/
│   ├── UpdateMemberProfile.cs
│   ├── UpdateMemberProfileHandler.cs
│   └── UpdateMemberProfileEndpoint.cs
│
├── TopUpBalance/
│   ├── TopUpBalance.cs
│   ├── TopUpBalanceHandler.cs
│   └── TopUpBalanceEndpoint.cs
│
├── DeductBalance/
│   ├── DeductBalance.cs                # 内部命令（不暴露 HTTP）
│   └── DeductBalanceHandler.cs
│
├── AwardPoints/
│   ├── AwardPoints.cs
│   └── AwardPointsHandler.cs
│
├── GetMember/
│   ├── GetMember.cs                    # Query
│   ├── GetMemberHandler.cs
│   └── GetMemberEndpoint.cs
│
├── GetMemberBalance/
│   ├── GetMemberBalance.cs
│   └── GetMemberBalanceHandler.cs
│
├── Events/
│   ├── MemberRegistered.cs            # 会员已注册事件
│   ├── BalanceToppedUp.cs             # 余额已充值事件
│   ├── BalanceDeducted.cs             # 余额已扣减事件
│   ├── PointsAwarded.cs               # 积分已赠送事件
│   └── MemberTierUpgraded.cs          # 会员等级已升级事件
│
├── Handlers/
│   └── PaymentCompletedHandler.cs     # 监听 PaymentCompleted 事件 → 计算积分
│
├── Member.cs                           # 聚合根
├── MemberTier.cs                       # 枚举：会员等级
└── MembersModule.cs                    # 模块标记（可选）
```

### 架构原则

- ✅ **一个功能 = 一个文件夹**: 如 `RegisterMember/` 包含该功能的所有代码
- ✅ **Handler 即应用服务**: 不再有 `IMemberAppService`
- ✅ **直接使用 Marten**: 不使用 `IRepository<Member>`
- ✅ **事件驱动通信**: 通过 Wolverine 消息总线发布/订阅事件

---

## 📊 领域模型

### Member（聚合根）

```csharp
public class Member
{
    public Guid Id { get; set; }
    public string Name { get; set; }
    public string Phone { get; set; }
    public string Email { get; set; }
    public MemberTier Tier { get; set; }
    public decimal Balance { get; set; }
    public int Points { get; set; }
    public DateTimeOffset RegisteredAt { get; set; }
    public DateTimeOffset? LastActiveAt { get; set; }

    // 领域方法
    public void TopUp(decimal amount)
    {
        if (amount <= 0)
            throw new InvalidOperationException("充值金额必须大于0");
        
        Balance += amount;
    }

    public void Deduct(decimal amount)
    {
        if (amount <= 0)
            throw new InvalidOperationException("扣减金额必须大于0");
        
        if (Balance < amount)
            throw new InvalidOperationException("余额不足");
        
        Balance -= amount;
    }

    public void AwardPoints(int points)
    {
        if (points <= 0)
            throw new InvalidOperationException("积分必须大于0");
        
        Points += points;
        CheckTierUpgrade(); // 检查是否需要升级
    }

    private void CheckTierUpgrade()
    {
        var newTier = Points switch
        {
            >= 10000 => MemberTier.Platinum,
            >= 5000 => MemberTier.Gold,
            >= 1000 => MemberTier.Silver,
            _ => MemberTier.Regular
        };

        if (newTier > Tier)
        {
            Tier = newTier;
        }
    }
}
```

### MemberTier（枚举）

```csharp
public enum MemberTier
{
    Regular = 1,   // 普通会员
    Silver = 2,    // 银卡会员
    Gold = 3,      // 金卡会员
    Platinum = 4   // 白金会员
}
```

---

## 🔧 核心功能切片

### 1. RegisterMember（注册会员）

#### Command 定义

```csharp
// RegisterMember.cs
namespace Zss.BilliardHall.Modules.Members.RegisterMember;

public sealed record RegisterMember(
    string Name,
    string Phone,
    string Email,
    string Password
);
```

#### Handler 实现

```csharp
// RegisterMemberHandler.cs
namespace Zss.BilliardHall.Modules.Members.RegisterMember;

public sealed class RegisterMemberHandler
{
    public async Task<Result<Guid>> Handle(
        RegisterMember command,
        IDocumentSession session,
        IPasswordHasher passwordHasher,
        IMessageBus bus,
        ILogger<RegisterMemberHandler> logger,
        CancellationToken ct = default)
    {
        // 1. 检查手机号是否已注册
        var existing = await session
            .Query<Member>()
            .FirstOrDefaultAsync(m => m.Phone == command.Phone, ct);

        if (existing != null)
            return Result.Fail<Guid>("手机号已注册");

        // 2. 创建会员
        var member = new Member
        {
            Id = Guid.NewGuid(),
            Name = command.Name,
            Phone = command.Phone,
            Email = command.Email,
            Tier = MemberTier.Regular,
            Balance = 0,
            Points = 0,
            RegisteredAt = DateTimeOffset.UtcNow
        };

        // 3. 持久化
        session.Store(member);
        await session.SaveChangesAsync(ct);

        // 4. 发布事件
        await bus.PublishAsync(new MemberRegistered(
            member.Id,
            member.Name,
            member.Phone,
            member.RegisteredAt
        ), ct);

        logger.LogInformation(
            "会员注册成功: {MemberId}, 手机号: {Phone}",
            member.Id,
            member.Phone
        );

        return Result.Ok(member.Id);
    }
}
```

#### HTTP 端点

```csharp
// RegisterMemberEndpoint.cs
namespace Zss.BilliardHall.Modules.Members.RegisterMember;

public sealed class RegisterMemberEndpoint
{
    [WolverinePost("/api/members/register")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public static async Task<IResult> Post(
        RegisterMemberRequest request,
        IMessageBus bus)
    {
        var command = new RegisterMember(
            request.Name,
            request.Phone,
            request.Email,
            request.Password
        );

        var result = await bus.InvokeAsync<Result<Guid>>(command);

        return result.IsSuccess
            ? Results.Ok(new { memberId = result.Value })
            : Results.BadRequest(new { error = result.Error });
    }

    public sealed record RegisterMemberRequest(
        string Name,
        string Phone,
        string Email,
        string Password
    );
}
```

#### 验证器

```csharp
// RegisterMemberValidator.cs
namespace Zss.BilliardHall.Modules.Members.RegisterMember;

public sealed class RegisterMemberValidator : AbstractValidator<RegisterMember>
{
    public RegisterMemberValidator()
    {
        RuleFor(x => x.Name)
            .NotEmpty().WithMessage("姓名不能为空")
            .MaximumLength(50).WithMessage("姓名不能超过50个字符");

        RuleFor(x => x.Phone)
            .NotEmpty().WithMessage("手机号不能为空")
            .Matches(@"^1[3-9]\d{9}$").WithMessage("手机号格式不正确");

        RuleFor(x => x.Email)
            .EmailAddress().WithMessage("邮箱格式不正确")
            .When(x => !string.IsNullOrEmpty(x.Email));

        RuleFor(x => x.Password)
            .NotEmpty().WithMessage("密码不能为空")
            .MinimumLength(6).WithMessage("密码至少6个字符");
    }
}
```

---

### 2. TopUpBalance（余额充值）

#### Command 定义

```csharp
// TopUpBalance.cs
namespace Zss.BilliardHall.Modules.Members.TopUpBalance;

public sealed record TopUpBalance(
    Guid MemberId,
    decimal Amount,
    string PaymentMethod
);
```

#### Handler 实现

```csharp
// TopUpBalanceHandler.cs
namespace Zss.BilliardHall.Modules.Members.TopUpBalance;

public sealed class TopUpBalanceHandler
{
    [Transactional]
    public async Task<Result> Handle(
        TopUpBalance command,
        IDocumentSession session,
        IMessageBus bus,
        ILogger<TopUpBalanceHandler> logger,
        CancellationToken ct = default)
    {
        // 1. 加载会员
        var member = await session.LoadAsync<Member>(command.MemberId, ct);
        if (member == null)
            return Result.Fail("会员不存在");

        // 2. 充值
        var oldBalance = member.Balance;
        member.TopUp(command.Amount);

        // 3. 持久化
        session.Store(member);
        await session.SaveChangesAsync(ct);

        // 4. 发布事件
        await bus.PublishAsync(new BalanceToppedUp(
            member.Id,
            command.Amount,
            oldBalance,
            member.Balance,
            DateTimeOffset.UtcNow
        ), ct);

        logger.LogInformation(
            "会员充值成功: {MemberId}, 金额: {Amount:F2}, 余额: {Balance:F2}",
            member.Id,
            command.Amount,
            member.Balance
        );

        return Result.Ok();
    }
}
```

---

### 3. DeductBalance（扣减余额）

> ⚠️ **内部命令**: 此命令不暴露 HTTP 端点，仅供其他模块通过消息总线调用

```csharp
// DeductBalance.cs
namespace Zss.BilliardHall.Modules.Members.DeductBalance;

public sealed record DeductBalance(
    Guid MemberId,
    decimal Amount,
    string Reason  // 扣减原因（如：支付订单、消费等）
);
```

```csharp
// DeductBalanceHandler.cs
namespace Zss.BilliardHall.Modules.Members.DeductBalance;

public sealed class DeductBalanceHandler
{
    [Transactional]
    public async Task<Result> Handle(
        DeductBalance command,
        IDocumentSession session,
        IMessageBus bus,
        ILogger<DeductBalanceHandler> logger,
        CancellationToken ct = default)
    {
        var member = await session.LoadAsync<Member>(command.MemberId, ct);
        if (member == null)
            return Result.Fail("会员不存在");

        // 扣减（会检查余额是否足够）
        try
        {
            var oldBalance = member.Balance;
            member.Deduct(command.Amount);

            session.Store(member);
            await session.SaveChangesAsync(ct);

            await bus.PublishAsync(new BalanceDeducted(
                member.Id,
                command.Amount,
                oldBalance,
                member.Balance,
                command.Reason,
                DateTimeOffset.UtcNow
            ), ct);

            logger.LogInformation(
                "会员余额扣减成功: {MemberId}, 金额: {Amount:F2}, 原因: {Reason}",
                member.Id,
                command.Amount,
                command.Reason
            );

            return Result.Ok();
        }
        catch (InvalidOperationException ex)
        {
            return Result.Fail(ex.Message);
        }
    }
}
```

---

### 4. GetMember（查询会员信息）

```csharp
// GetMember.cs
namespace Zss.BilliardHall.Modules.Members.GetMember;

public sealed record GetMember(Guid MemberId);

public sealed record MemberDto(
    Guid Id,
    string Name,
    string Phone,
    string Email,
    MemberTier Tier,
    decimal Balance,
    int Points,
    DateTimeOffset RegisteredAt
);
```

```csharp
// GetMemberHandler.cs
namespace Zss.BilliardHall.Modules.Members.GetMember;

public sealed class GetMemberHandler
{
    public async Task<Result<MemberDto>> Handle(
        GetMember query,
        IDocumentSession session,
        CancellationToken ct = default)
    {
        var member = await session.LoadAsync<Member>(query.MemberId, ct);
        
        if (member == null)
            return Result.Fail<MemberDto>("会员不存在");

        var dto = new MemberDto(
            member.Id,
            member.Name,
            member.Phone,
            member.Email,
            member.Tier,
            member.Balance,
            member.Points,
            member.RegisteredAt
        );

        return Result.Ok(dto);
    }
}
```

```csharp
// GetMemberEndpoint.cs
namespace Zss.BilliardHall.Modules.Members.GetMember;

public sealed class GetMemberEndpoint
{
    [WolverineGet("/api/members/{memberId:guid}")]
    public static async Task<IResult> Get(
        Guid memberId,
        IMessageBus bus)
    {
        var result = await bus.InvokeAsync<Result<MemberDto>>(
            new GetMember(memberId)
        );

        return result.IsSuccess
            ? Results.Ok(result.Value)
            : Results.NotFound(new { error = result.Error });
    }
}
```

---

## 🔔 领域事件

### MemberRegistered（会员已注册）

```csharp
// Events/MemberRegistered.cs
namespace Zss.BilliardHall.Modules.Members.Events;

public sealed record MemberRegistered(
    Guid MemberId,
    string Name,
    string Phone,
    DateTimeOffset RegisteredAt
);
```

**订阅者示例**（其他模块）:
```csharp
// 在其他模块中监听
public class MemberRegisteredHandler
{
    public async Task Handle(
        MemberRegistered @event,
        ILogger<MemberRegisteredHandler> logger)
    {
        // 发送欢迎邮件、短信等
        logger.LogInformation(
            "新会员注册: {MemberName}, 手机号: {Phone}",
            @event.Name,
            @event.Phone
        );
    }
}
```

### BalanceToppedUp（余额已充值）

```csharp
// Events/BalanceToppedUp.cs
namespace Zss.BilliardHall.Modules.Members.Events;

public sealed record BalanceToppedUp(
    Guid MemberId,
    decimal Amount,
    decimal OldBalance,
    decimal NewBalance,
    DateTimeOffset ToppedUpAt
);
```

### BalanceDeducted（余额已扣减）

```csharp
// Events/BalanceDeducted.cs
namespace Zss.BilliardHall.Modules.Members.Events;

public sealed record BalanceDeducted(
    Guid MemberId,
    decimal Amount,
    decimal OldBalance,
    decimal NewBalance,
    string Reason,
    DateTimeOffset DeductedAt
);
```

---

## 🔗 跨模块通信

### 监听外部事件

#### PaymentCompletedHandler（监听支付完成事件）

```csharp
// Handlers/PaymentCompletedHandler.cs
namespace Zss.BilliardHall.Modules.Members.Handlers;

public sealed class PaymentCompletedHandler
{
    public async Task Handle(
        PaymentCompleted @event,  // 来自 Payments 模块
        IDocumentSession session,
        IMessageBus bus,
        ILogger<PaymentCompletedHandler> logger,
        CancellationToken ct = default)
    {
        if (@event.MemberId == null)
            return; // 散客，无需处理

        // 计算积分（消费 1 元 = 1 积分）
        var points = (int)Math.Floor(@event.Amount);

        // 发送赠送积分命令
        await bus.InvokeAsync(new AwardPoints(
            @event.MemberId.Value,
            points,
            $"消费赠送：订单 {@event.OrderId}"
        ), ct);

        logger.LogInformation(
            "支付完成，赠送积分: {MemberId}, 积分: {Points}",
            @event.MemberId,
            points
        );
    }
}
```

### 被其他模块调用

其他模块通过消息总线调用 Members 模块的命令：

```csharp
// 在 Payments 模块中扣减会员余额
public class ProcessPaymentHandler
{
    public async Task Handle(
        ProcessPayment command,
        IMessageBus bus)
    {
        if (command.PaymentMethod == PaymentMethod.MemberBalance)
        {
            // 调用 Members 模块扣减余额
            var result = await bus.InvokeAsync<Result>(
                new DeductBalance(
                    command.MemberId,
                    command.Amount,
                    $"支付订单: {command.OrderId}"
                )
            );

            if (!result.IsSuccess)
            {
                // 处理余额不足等错误
                throw new PaymentException(result.Error);
            }
        }
    }
}
```

---

## 🧪 测试示例

### 单元测试

```csharp
public class RegisterMemberHandlerTests
{
    [Fact]
    public async Task Should_Register_Member_Successfully()
    {
        // Arrange
        await using var store = DocumentStore.For(opts =>
        {
            opts.Connection(ConnectionSource.InMemoryConnectionString);
            opts.AutoCreateSchemaObjects = AutoCreate.All;
        });

        await using var session = store.LightweightSession();
        var bus = Substitute.For<IMessageBus>();
        var passwordHasher = Substitute.For<IPasswordHasher>();
        var logger = Substitute.For<ILogger<RegisterMemberHandler>>();

        var handler = new RegisterMemberHandler();
        var command = new RegisterMember(
            "张三",
            "13800138000",
            "zhangsan@example.com",
            "password123"
        );

        // Act
        var result = await handler.Handle(
            command,
            session,
            passwordHasher,
            bus,
            logger
        );

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().NotBeEmpty();

        var member = await session.LoadAsync<Member>(result.Value);
        member.Should().NotBeNull();
        member.Name.Should().Be("张三");
        member.Phone.Should().Be("13800138000");
        member.Tier.Should().Be(MemberTier.Regular);
        member.Balance.Should().Be(0);
    }

    [Fact]
    public async Task Should_Fail_When_Phone_Already_Registered()
    {
        // Arrange
        await using var store = DocumentStore.For(opts =>
        {
            opts.Connection(ConnectionSource.InMemoryConnectionString);
            opts.AutoCreateSchemaObjects = AutoCreate.All;
        });

        await using var session = store.LightweightSession();

        // 预先创建一个会员
        var existingMember = new Member
        {
            Id = Guid.NewGuid(),
            Phone = "13800138000",
            Name = "已存在的会员"
        };
        session.Store(existingMember);
        await session.SaveChangesAsync();

        var handler = new RegisterMemberHandler();
        var command = new RegisterMember(
            "张三",
            "13800138000",  // 相同手机号
            "zhangsan@example.com",
            "password123"
        );

        // Act
        var result = await handler.Handle(
            command,
            session,
            Substitute.For<IPasswordHasher>(),
            Substitute.For<IMessageBus>(),
            Substitute.For<ILogger<RegisterMemberHandler>>()
        );

        // Assert
        result.IsSuccess.Should().BeFalse();
        result.Error.Should().Contain("已注册");
    }
}
```

---

## 📝 开发规范

### 命名约定

- Command: 动词 + 名词（`RegisterMember`, `TopUpBalance`）
- Query: Get/Find/List + 名词（`GetMember`, `ListMembers`）
- Event: 名词 + 动词过去式（`MemberRegistered`, `BalanceToppedUp`）
- Handler: 消息名 + Handler（`RegisterMemberHandler`）

### 验证规则

- 使用 FluentValidation 进行输入验证
- Handler 中只处理业务规则验证
- 领域模型中包含不变量验证（如 `Deduct` 方法检查余额）

### 日志规范

```csharp
// ✅ 结构化日志
logger.LogInformation(
    "会员注册成功: {MemberId}, 手机号: {Phone}",
    member.Id,
    member.Phone
);

// ❌ 避免字符串拼接
logger.LogInformation($"会员注册成功: {member.Id}");
```

### 错误处理

- 使用 `Result<T>` 类型返回结果
- 领域异常用 `InvalidOperationException`
- 不在 Handler 中直接抛出业务异常（返回 Result.Fail）

---

## 🔗 相关文档

### 核心架构

- **[Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)** ⭐⭐⭐
- **[系统模块划分 - Members](../03_系统架构设计/系统模块划分.md#7-members-模块会员体系)** ⭐⭐
- **[Wolverine快速上手指南](../03_系统架构设计/Wolverine快速上手指南.md)** ⭐⭐⭐

### 相关模块

- **Payments 模块**: 支付完成后计算积分
- **Sessions 模块**: 散客 vs. 会员时段管理
- **Billing 模块**: 会员折扣计算

### 历史文档

- **[会员管理模块 v1（legacy）](legacy/会员管理模块_v1.md)**: 基于 ABP 分层架构的旧版本文档，仅供参考

---

*最后更新: 2024-01-15 | 版本: v3.0.0 | 架构: Wolverine + 垂直切片*
