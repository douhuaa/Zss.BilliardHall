...
migration_status: "核心模块已完整重写为独立文档"
---
...
| ⏱️ [打球时段模块](打球时段模块.md) | 时段管理、Saga 工作流 | TableSessionSaga、时长计算 | 核心业务模块 | ✅ v2.0.0（独立文档）|
| 💰 [计费管理模块](计费管理模块.md) | 计费规则、账单管理 | 计费算法、价格策略 | 核心业务模块 | ✅ v2.0.0（独立文档）|
| 💳 [支付模块](支付模块.md) | 支付流程、退款管理 | 幂等性、事务处理 | 支付集成必读 | 🔄 待重写 |
| 🖥️ [设备管理模块](设备管理模块.md) | 设备控制、状态监控 | 物联网协议、实时通信 | 硬件对接必读 | 🔄 待重写 |
| 📊 [报表与统计模块](报表与统计模块.md) | 数据分析、报表生成 | 数据聚合、可视化 | 数据分析参考 | 🔄 待重写 |

**版本说明**:
- ✅ **v3.0.0**: 完全基于 Wolverine 架构重写，无旧架构内容（如 Members 模块）
- ✅ **v2.0.0**: 完全基于 Wolverine 架构重写，独立文档（如 Sessions、Billing 模块）
- 🔄 **待重写**: 待后续完成 Wolverine 架构迁移

> 💡 **提示**: v3.0.0 和 v2.0.0 文档完全基于 Wolverine 垂直切片架构，提供完整的代码示例和测试用例，可直接用于开发参考
...
详见：[Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)（序章：垂直切片架构理念）
...
- **[Wolverine模块化架构蓝图](../03_系统架构设计/Wolverine模块化架构蓝图.md)** - 完整的可落地实施指南
  - 包含垂直切片架构理念、框架特性详解、与MediatR对比、迁移指南等
