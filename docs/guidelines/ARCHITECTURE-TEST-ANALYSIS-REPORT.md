...
**P0 (1-2 天)**: 消除剩余 3 个 FindRepositoryRoot 重复  
...
**成效**：已消除 ~1,640 行重复代码，从 84 个文件降至 3 个

| 类别 | 状态 |
|------|------|
| 使用 TestEnvironment | 82 个文件（65.6%）✅ |
| 仍需迁移 | ADR_301、ADR_360（共 3 个文件）|
| 预期收益 | 再减少 ~60 行重复代码 |
...
- [ ] 消除剩余 3 个 FindRepositoryRoot（ADR_301、ADR_360）
...
### 🎯 下一步行动
1. **短期**（1-2 周）：将 FileSystemTestHelper 和 AssertionMessageBuilder 采用率提升至 50%
