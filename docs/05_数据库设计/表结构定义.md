---
title: "表结构定义"
description: "EF Core 实体建模规范和数据库表结构定义"
section: "5.2"
version: "1.0.0"
author: "数据库团队"
maintainer: "开发工程师"
...
level: "必读"
audience: ["开发工程师", "DBA"]
keywords: ["表结构", "实体建模", "EF Core实体", "DbContext配置", "数据库映射", "ABP框架"]
...
**导航路径**: [🏠 项目首页](../../README.md) > [📊 数据库设计](README.md) > 🏗️ 表结构定义

<!-- Keywords for Search -->
**关键词**: `表结构` `实体建模` `EF Core实体` `DbContext配置` `数据库映射` `ABP框架`

## 概述

本文档定义了自助台球系统的数据库表结构，基于 EF Core Code First 方式设计。所有实体类都继承自 ABP 框架的基类，提供审计、软删除等通用功能。

> 💡 **相关章节**：
> - 业务逻辑详细说明请参考 [5.3 关键表说明](关键表说明.md)
> - 数据库迁移操作请参考 [5.5 数据迁移方案](数据迁移方案.md)
> - 完整开发流程请参考 [5.6 EF Core 开发工作流程](EFCore开发工作流程.md)
...
public class BillingSnapshot : Entity<long>
{
    ...
    public string PricingRules { get; set; } // JSON格式存储定价规则
...
public class DeviceHeartbeat : Entity<long>
{
    ...
    public string MetricsJson { get; set; } // JSON格式存储设备指标
