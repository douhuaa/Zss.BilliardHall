...
- 根据 ADR-900，方法名或 DisplayName 应包含 `ADR-{编号}`
- 清晰表达验证的约束内容

#### C. 测试类头部注释

根据 ADR-900，每个测试类**应**包含 ADR 映射清单：
...
根据 ADR-900，每个 `adr-XXXX.prompts.md` 文件应包含 **"测试覆盖自检"** 章节：
...
1. ✅ 根据 ADR-900，所有需要测试覆盖的约束是否都有对应的测试方法？
...
1. **扫描 ADR 文档**：
  - 提取所有标记为测试覆盖的约束（根据 ADR-900 定义的标记格式）
  - 统计需要测试的约束数量
...
1. 根据 ADR-900，检查是否所有需要测试覆盖的约束都有对应测试
...
**A**: 不是。根据 ADR-900，只有标记为需要测试覆盖的约束才需要自动化测试。以下情况不需要：
